var gradeDict = crossfilter([{'grade': 'AB3',
  'index': '0-2',
  'level': 'A',
  'name': 'e1621e08fc',
  'natScore': 4,
  'order': 1,
  'subject': 'Irish'},
 {'grade': 'AB2',
  'index': '0-3',
  'level': 'A',
  'name': 'e1621e08fc',
  'natScore': 15,
  'order': 1,
  'subject': 'English'},
 {'grade': 'GC2',
  'index': '0-4',
  'level': 'G',
  'name': 'e1621e08fc',
  'natScore': 15,
  'order': 1,
  'subject': 'Maths'},
 {'grade': 'AB3',
  'index': '0-8',
  'level': 'A',
  'name': 'e1621e08fc',
  'natScore': 21,
  'order': 1,
  'subject': 'German'},
 {'grade': 'AC1',
  'index': '0-14',
  'level': 'A',
  'name': 'e1621e08fc',
  'natScore': 36,
  'order': 1,
  'subject': 'Ag. Science'},
 {'grade': 'AC2',
  'index': '0-15',
  'level': 'A',
  'name': 'e1621e08fc',
  'natScore': 79,
  'order': 1,
  'subject': 'Biology'},
 {'grade': 'AA2',
  'index': '0-21',
  'level': 'A',
  'name': 'e1621e08fc',
  'natScore': 55,
  'order': 1,
  'subject': 'Music'},
 {'grade': 'AB1',
  'index': '1-2',
  'level': 'A',
  'name': '2abae8c077',
  'natScore': 70,
  'order': 1,
  'subject': 'Irish'},
 {'grade': 'AC1',
  'index': '1-3',
  'level': 'A',
  'name': '2abae8c077',
  'natScore': 99,
  'order': 1,
  'subject': 'English'},
 {'grade': 'AC2',
  'index': '1-4',
  'level': 'A',
  'name': '2abae8c077',
  'natScore': 13,
  'order': 1,
  'subject': 'Maths'},
 {'grade': 'AC1',
  'index': '1-9',
  'level': 'A',
  'name': '2abae8c077',
  'natScore': 85,
  'order': 1,
  'subject': 'Spanish'},
 {'grade': 'AB1',
  'index': '1-15',
  'level': 'A',
  'name': '2abae8c077',
  'natScore': 6,
  'order': 1,
  'subject': 'Biology'},
 {'grade': 'AB2',
  'index': '1-18',
  'level': 'A',
  'name': '2abae8c077',
  'natScore': 7,
  'order': 1,
  'subject': 'Accounting'},
 {'grade': 'AB1',
  'index': '1-21',
  'level': 'A',
  'name': '2abae8c077',
  'natScore': 37,
  'order': 1,
  'subject': 'Music'},
 {'grade': 'AC1',
  'index': '2-2',
  'level': 'A',
  'name': '8e610d191f',
  'natScore': 53,
  'order': 1,
  'subject': 'Irish'},
 {'grade': 'AB1',
  'index': '2-3',
  'level': 'A',
  'name': '8e610d191f',
  'natScore': 83,
  'order': 1,
  'subject': 'English'},
 {'grade': 'AC2',
  'index': '2-4',
  'level': 'A',
  'name': '8e610d191f',
  'natScore': 58,
  'order': 1,
  'subject': 'Maths'},
 {'grade': 'AB2',
  'index': '2-5',
  'level': 'A',
  'name': '8e610d191f',
  'natScore': 86,
  'order': 1,
  'subject': 'History'},
 {'grade': 'AB1',
  'index': '2-8',
  'level': 'A',
  'name': '8e610d191f',
  'natScore': 66,
  'order': 1,
  'subject': 'German'},
 {'grade': 'AB2',
  'index': '2-14',
  'level': 'A',
  'name': '8e610d191f',
  'natScore': 58,
  'order': 1,
  'subject': 'Ag. Science'},
 {'grade': 'AC1',
  'index': '2-15',
  'level': 'A',
  'name': '8e610d191f',
  'natScore': 87,
  'order': 1,
  'subject': 'Biology'},
 {'grade': 'AB1',
  'index': '2-21',
  'level': 'A',
  'name': '8e610d191f',
  'natScore': 66,
  'order': 1,
  'subject': 'Music'},
 {'grade': 'GD1',
  'index': '3-3',
  'level': 'G',
  'name': '07de7185d0',
  'natScore': 29,
  'order': 1,
  'subject': 'English'},
 {'grade': 'GD1',
  'index': '3-6',
  'level': 'G',
  'name': '07de7185d0',
  'natScore': 89,
  'order': 1,
  'subject': 'Geography'},
 {'grade': 'GD3',
  'index': '3-10',
  'level': 'G',
  'name': '07de7185d0',
  'natScore': 90,
  'order': 1,
  'subject': 'Art'},
 {'grade': 'AD2',
  'index': '3-16',
  'level': 'A',
  'name': '07de7185d0',
  'natScore': 67,
  'order': 1,
  'subject': 'Engineering'},
 {'grade': 'AE',
  'index': '3-17',
  'level': 'A',
  'name': '07de7185d0',
  'natScore': 68,
  'order': 1,
  'subject': 'Construction Studies'},
 {'grade': 'AA2',
  'index': '4-2',
  'level': 'A',
  'name': 'c0097be504',
  'natScore': 59,
  'order': 1,
  'subject': 'Irish'},
 {'grade': 'AA2',
  'index': '4-3',
  'level': 'A',
  'name': 'c0097be504',
  'natScore': 15,
  'order': 1,
  'subject': 'English'},
 {'grade': 'AA2',
  'index': '4-4',
  'level': 'A',
  'name': 'c0097be504',
  'natScore': 13,
  'order': 1,
  'subject': 'Maths'},
 {'grade': 'AA1',
  'index': '4-8',
  'level': 'A',
  'name': 'c0097be504',
  'natScore': 48,
  'order': 1,
  'subject': 'German'},
 {'grade': 'AA1',
  'index': '4-13',
  'level': 'A',
  'name': 'c0097be504',
  'natScore': 49,
  'order': 1,
  'subject': 'Chemistry'},
 {'grade': 'AA1',
  'index': '4-15',
  'level': 'A',
  'name': 'c0097be504',
  'natScore': 39,
  'order': 1,
  'subject': 'Biology'},
 {'grade': 'AA1',
  'index': '4-20',
  'level': 'A',
  'name': 'c0097be504',
  'natScore': 46,
  'order': 1,
  'subject': 'Economics'},
 {'grade': 'AD2',
  'index': '5-3',
  'level': 'A',
  'name': '30b3497923',
  'natScore': 23,
  'order': 1,
  'subject': 'English'},
 {'grade': 'AD3',
  'index': '5-5',
  'level': 'A',
  'name': '30b3497923',
  'natScore': 58,
  'order': 1,
  'subject': 'History'},
 {'grade': 'AD3',
  'index': '5-6',
  'level': 'A',
  'name': '30b3497923',
  'natScore': 92,
  'order': 1,
  'subject': 'Geography'},
 {'grade': 'GE',
  'index': '5-7',
  'level': 'G',
  'name': '30b3497923',
  'natScore': 84,
  'order': 1,
  'subject': 'French'},
 {'grade': 'GE',
  'index': '5-15',
  'level': 'G',
  'name': '30b3497923',
  'natScore': 34,
  'order': 1,
  'subject': 'Biology'},
 {'grade': 'GB1',
  'index': '6-2',
  'level': 'G',
  'name': '609d94219b',
  'natScore': 66,
  'order': 1,
  'subject': 'Irish'},
 {'grade': 'AD3',
  'index': '6-3',
  'level': 'A',
  'name': '609d94219b',
  'natScore': 99,
  'order': 1,
  'subject': 'English'},
 {'grade': 'GA2',
  'index': '6-4',
  'level': 'G',
  'name': '609d94219b',
  'natScore': 46,
  'order': 1,
  'subject': 'Maths'},
 {'grade': 'AC2',
  'index': '6-8',
  'level': 'A',
  'name': '609d94219b',
  'natScore': 36,
  'order': 1,
  'subject': 'German'},
 {'grade': 'GB3',
  'index': '6-13',
  'level': 'G',
  'name': '609d94219b',
  'natScore': 86,
  'order': 1,
  'subject': 'Chemistry'},
 {'grade': 'AD1',
  'index': '6-14',
  'level': 'A',
  'name': '609d94219b',
  'natScore': 59,
  'order': 1,
  'subject': 'Ag. Science'},
 {'grade': 'AC2',
  'index': '6-15',
  'level': 'A',
  'name': '609d94219b',
  'natScore': 47,
  'order': 1,
  'subject': 'Biology'},
 {'grade': 'GB3',
  'index': '7-2',
  'level': 'G',
  'name': 'f9a4e01cf7',
  'natScore': 22,
  'order': 1,
  'subject': 'Irish'},
 {'grade': 'AA2',
  'index': '7-3',
  'level': 'A',
  'name': 'f9a4e01cf7',
  'natScore': 65,
  'order': 1,
  'subject': 'English'},
 {'grade': 'GB3',
  'index': '7-4',
  'level': 'G',
  'name': 'f9a4e01cf7',
  'natScore': 31,
  'order': 1,
  'subject': 'Maths'},
 {'grade': 'AB1',
  'index': '7-5',
  'level': 'A',
  'name': 'f9a4e01cf7',
  'natScore': 66,
  'order': 1,
  'subject': 'History'},
 {'grade': 'AC1',
  'index': '7-7',
  'level': 'A',
  'name': 'f9a4e01cf7',
  'natScore': 97,
  'order': 1,
  'subject': 'French'},
 {'grade': 'AB2',
  'index': '7-15',
  'level': 'A',
  'name': 'f9a4e01cf7',
  'natScore': 41,
  'order': 1,
  'subject': 'Biology'},
 {'grade': 'AB1',
  'index': '7-21',
  'level': 'A',
  'name': 'f9a4e01cf7',
  'natScore': 42,
  'order': 1,
  'subject': 'Music'},
 {'grade': 'AA1',
  'index': '8-2',
  'level': 'A',
  'name': '33eed78259',
  'natScore': 22,
  'order': 1,
  'subject': 'Irish'},
 {'grade': 'AA2',
  'index': '8-3',
  'level': 'A',
  'name': '33eed78259',
  'natScore': 80,
  'order': 1,
  'subject': 'English'},
 {'grade': 'GA1',
  'index': '8-4',
  'level': 'G',
  'name': '33eed78259',
  'natScore': 87,
  'order': 1,
  'subject': 'Maths'},
 {'grade': 'AB2',
  'index': '8-5',
  'level': 'A',
  'name': '33eed78259',
  'natScore': 25,
  'order': 1,
  'subject': 'History'},
 {'grade': 'AA1',
  'index': '8-8',
  'level': 'A',
  'name': '33eed78259',
  'natScore': 36,
  'order': 1,
  'subject': 'German'},
 {'grade': 'AB3',
  'index': '8-15',
  'level': 'A',
  'name': '33eed78259',
  'natScore': 37,
  'order': 1,
  'subject': 'Biology'},
 {'grade': 'AA1',
  'index': '8-21',
  'level': 'A',
  'name': '33eed78259',
  'natScore': 52,
  'order': 1,
  'subject': 'Music'},
 {'grade': 'AB3',
  'index': '9-2',
  'level': 'A',
  'name': 'aeec8497c9',
  'natScore': 44,
  'order': 1,
  'subject': 'Irish'},
 {'grade': 'AB1',
  'index': '9-3',
  'level': 'A',
  'name': 'aeec8497c9',
  'natScore': 63,
  'order': 1,
  'subject': 'English'},
 {'grade': 'GB2',
  'index': '9-4',
  'level': 'G',
  'name': 'aeec8497c9',
  'natScore': 70,
  'order': 1,
  'subject': 'Maths'},
 {'grade': 'AC2',
  'index': '9-9',
  'level': 'A',
  'name': 'aeec8497c9',
  'natScore': 10,
  'order': 1,
  'subject': 'Spanish'},
 {'grade': 'AD2',
  'index': '9-13',
  'level': 'A',
  'name': 'aeec8497c9',
  'natScore': 77,
  'order': 1,
  'subject': 'Chemistry'},
 {'grade': 'AC3',
  'index': '9-15',
  'level': 'A',
  'name': 'aeec8497c9',
  'natScore': 23,
  'order': 1,
  'subject': 'Biology'},
 {'grade': 'AB2',
  'index': '9-21',
  'level': 'A',
  'name': 'aeec8497c9',
  'natScore': 47,
  'order': 1,
  'subject': 'Music'},
 {'grade': 'GB2',
  'index': '10-2',
  'level': 'G',
  'name': 'ec1c0c59ec',
  'natScore': 97,
  'order': 1,
  'subject': 'Irish'},
 {'grade': 'GB1',
  'index': '10-3',
  'level': 'G',
  'name': 'ec1c0c59ec',
  'natScore': 13,
  'order': 1,
  'subject': 'English'},
 {'grade': 'AC1',
  'index': '10-4',
  'level': 'A',
  'name': 'ec1c0c59ec',
  'natScore': 30,
  'order': 1,
  'subject': 'Maths'},
 {'grade': 'GB1',
  'index': '10-7',
  'level': 'G',
  'name': 'ec1c0c59ec',
  'natScore': 65,
  'order': 1,
  'subject': 'French'},
 {'grade': 'AB2',
  'index': '10-14',
  'level': 'A',
  'name': 'ec1c0c59ec',
  'natScore': 38,
  'order': 1,
  'subject': 'Ag. Science'},
 {'grade': 'AC2',
  'index': '10-15',
  'level': 'A',
  'name': 'ec1c0c59ec',
  'natScore': 15,
  'order': 1,
  'subject': 'Biology'},
 {'grade': 'AA2',
  'index': '10-16',
  'level': 'A',
  'name': 'ec1c0c59ec',
  'natScore': 47,
  'order': 1,
  'subject': 'Engineering'},
 {'grade': 'GB1',
  'index': '11-2',
  'level': 'G',
  'name': 'a9de1dc084',
  'natScore': 22,
  'order': 1,
  'subject': 'Irish'},
 {'grade': 'GA2',
  'index': '11-3',
  'level': 'G',
  'name': 'a9de1dc084',
  'natScore': 72,
  'order': 1,
  'subject': 'English'},
 {'grade': 'AC3',
  'index': '11-4',
  'level': 'A',
  'name': 'a9de1dc084',
  'natScore': 82,
  'order': 1,
  'subject': 'Maths'},
 {'grade': 'AC1',
  'index': '11-6',
  'level': 'A',
  'name': 'a9de1dc084',
  'natScore': 39,
  'order': 1,
  'subject': 'Geography'},
 {'grade': 'AD1',
  'index': '11-8',
  'level': 'A',
  'name': 'a9de1dc084',
  'natScore': 42,
  'order': 1,
  'subject': 'German'},
 {'grade': 'AC2',
  'index': '11-10',
  'level': 'A',
  'name': 'a9de1dc084',
  'natScore': 67,
  'order': 1,
  'subject': 'Art'},
 {'grade': 'AB1',
  'index': '11-21',
  'level': 'A',
  'name': 'a9de1dc084',
  'natScore': 85,
  'order': 1,
  'subject': 'Music'},
 {'grade': 'AA1',
  'index': '12-2',
  'level': 'A',
  'name': '6d5bcda33b',
  'natScore': 45,
  'order': 1,
  'subject': 'Irish'},
 {'grade': 'AA2',
  'index': '12-3',
  'level': 'A',
  'name': '6d5bcda33b',
  'natScore': 21,
  'order': 1,
  'subject': 'English'},
 {'grade': 'AA1',
  'index': '12-4',
  'level': 'A',
  'name': '6d5bcda33b',
  'natScore': 25,
  'order': 1,
  'subject': 'Maths'},
 {'grade': 'AB1',
  'index': '12-9',
  'level': 'A',
  'name': '6d5bcda33b',
  'natScore': 29,
  'order': 1,
  'subject': 'Spanish'},
 {'grade': 'AB2',
  'index': '12-10',
  'level': 'A',
  'name': '6d5bcda33b',
  'natScore': 24,
  'order': 1,
  'subject': 'Art'},
 {'grade': 'AA1',
  'index': '12-11',
  'level': 'A',
  'name': '6d5bcda33b',
  'natScore': 81,
  'order': 1,
  'subject': 'Applied Maths'},
 {'grade': 'AA2',
  'index': '12-12',
  'level': 'A',
  'name': '6d5bcda33b',
  'natScore': 37,
  'order': 1,
  'subject': 'Physics'},
 {'grade': 'AC1',
  'index': '12-18',
  'level': 'A',
  'name': '6d5bcda33b',
  'natScore': 62,
  'order': 1,
  'subject': 'Accounting'},
 {'grade': 'GB3',
  'index': '13-2',
  'level': 'G',
  'name': 'c800e6f995',
  'natScore': 35,
  'order': 1,
  'subject': 'Irish'},
 {'grade': 'GB2',
  'index': '13-3',
  'level': 'G',
  'name': 'c800e6f995',
  'natScore': 15,
  'order': 1,
  'subject': 'English'},
 {'grade': 'GC1',
  'index': '13-4',
  'level': 'G',
  'name': 'c800e6f995',
  'natScore': 19,
  'order': 1,
  'subject': 'Maths'},
 {'grade': 'AD1',
  'index': '13-6',
  'level': 'A',
  'name': 'c800e6f995',
  'natScore': 95,
  'order': 1,
  'subject': 'Geography'},
 {'grade': 'AD2',
  'index': '13-15',
  'level': 'A',
  'name': 'c800e6f995',
  'natScore': 60,
  'order': 1,
  'subject': 'Biology'},
 {'grade': 'AB3',
  'index': '13-16',
  'level': 'A',
  'name': 'c800e6f995',
  'natScore': 5,
  'order': 1,
  'subject': 'Engineering'},
 {'grade': 'AC1',
  'index': '13-17',
  'level': 'A',
  'name': 'c800e6f995',
  'natScore': 76,
  'order': 1,
  'subject': 'Construction Studies'},
 {'grade': 'GC3',
  'index': '14-2',
  'level': 'G',
  'name': 'efaaa2107e',
  'natScore': 25,
  'order': 1,
  'subject': 'Irish'},
 {'grade': 'GC3',
  'index': '14-3',
  'level': 'G',
  'name': 'efaaa2107e',
  'natScore': 76,
  'order': 1,
  'subject': 'English'},
 {'grade': 'GE',
  'index': '14-4',
  'level': 'G',
  'name': 'efaaa2107e',
  'natScore': 20,
  'order': 1,
  'subject': 'Maths'},
 {'grade': 'GD1',
  'index': '14-6',
  'level': 'G',
  'name': 'efaaa2107e',
  'natScore': 31,
  'order': 1,
  'subject': 'Geography'},
 {'grade': 'GD3',
  'index': '14-9',
  'level': 'G',
  'name': 'efaaa2107e',
  'natScore': 8,
  'order': 1,
  'subject': 'Spanish'},
 {'grade': 'AD2',
  'index': '14-16',
  'level': 'A',
  'name': 'efaaa2107e',
  'natScore': 76,
  'order': 1,
  'subject': 'Engineering'},
 {'grade': 'GD3',
  'index': '15-2',
  'level': 'G',
  'name': 'c435ec1ca6',
  'natScore': 17,
  'order': 1,
  'subject': 'Irish'},
 {'grade': 'GB3',
  'index': '15-3',
  'level': 'G',
  'name': 'c435ec1ca6',
  'natScore': 35,
  'order': 1,
  'subject': 'English'},
 {'grade': 'GC1',
  'index': '15-4',
  'level': 'G',
  'name': 'c435ec1ca6',
  'natScore': 35,
  'order': 1,
  'subject': 'Maths'},
 {'grade': 'AD1',
  'index': '15-5',
  'level': 'A',
  'name': 'c435ec1ca6',
  'natScore': 60,
  'order': 1,
  'subject': 'History'},
 {'grade': 'AC3',
  'index': '15-6',
  'level': 'A',
  'name': 'c435ec1ca6',
  'natScore': 3,
  'order': 1,
  'subject': 'Geography'},
 {'grade': 'AD3',
  'index': '15-15',
  'level': 'A',
  'name': 'c435ec1ca6',
  'natScore': 54,
  'order': 1,
  'subject': 'Biology'},
 {'grade': 'AD3',
  'index': '15-16',
  'level': 'A',
  'name': 'c435ec1ca6',
  'natScore': 65,
  'order': 1,
  'subject': 'Engineering'},
 {'grade': 'GC3',
  'index': '16-2',
  'level': 'G',
  'name': '3806a3559b',
  'natScore': 16,
  'order': 1,
  'subject': 'Irish'},
 {'grade': 'GB2',
  'index': '16-3',
  'level': 'G',
  'name': '3806a3559b',
  'natScore': 54,
  'order': 1,
  'subject': 'English'},
 {'grade': 'GB3',
  'index': '16-4',
  'level': 'G',
  'name': '3806a3559b',
  'natScore': 24,
  'order': 1,
  'subject': 'Maths'},
 {'grade': 'AC3',
  'index': '16-6',
  'level': 'A',
  'name': '3806a3559b',
  'natScore': 73,
  'order': 1,
  'subject': 'Geography'},
 {'grade': 'AD1',
  'index': '16-15',
  'level': 'A',
  'name': '3806a3559b',
  'natScore': 70,
  'order': 1,
  'subject': 'Biology'},
 {'grade': 'AB2',
  'index': '16-16',
  'level': 'A',
  'name': '3806a3559b',
  'natScore': 32,
  'order': 1,
  'subject': 'Engineering'},
 {'grade': 'AB2',
  'index': '16-17',
  'level': 'A',
  'name': '3806a3559b',
  'natScore': 73,
  'order': 1,
  'subject': 'Construction Studies'},
 {'grade': 'AB3',
  'index': '17-2',
  'level': 'A',
  'name': 'e2af0d5bff',
  'natScore': 64,
  'order': 1,
  'subject': 'Irish'},
 {'grade': 'AB1',
  'index': '17-3',
  'level': 'A',
  'name': 'e2af0d5bff',
  'natScore': 64,
  'order': 1,
  'subject': 'English'},
 {'grade': 'GC1',
  'index': '17-4',
  'level': 'G',
  'name': 'e2af0d5bff',
  'natScore': 78,
  'order': 1,
  'subject': 'Maths'},
 {'grade': 'AA2',
  'index': '17-5',
  'level': 'A',
  'name': 'e2af0d5bff',
  'natScore': 2,
  'order': 1,
  'subject': 'History'},
 {'grade': 'AC2',
  'index': '17-8',
  'level': 'A',
  'name': 'e2af0d5bff',
  'natScore': 55,
  'order': 1,
  'subject': 'German'},
 {'grade': 'AD2',
  'index': '17-15',
  'level': 'A',
  'name': 'e2af0d5bff',
  'natScore': 5,
  'order': 1,
  'subject': 'Biology'},
 {'grade': 'AC2',
  'index': '17-21',
  'level': 'A',
  'name': 'e2af0d5bff',
  'natScore': 40,
  'order': 1,
  'subject': 'Music'},
 {'grade': 'GD2',
  'index': '18-2',
  'level': 'G',
  'name': '088198ee59',
  'natScore': 56,
  'order': 1,
  'subject': 'Irish'},
 {'grade': 'GB3',
  'index': '18-3',
  'level': 'G',
  'name': '088198ee59',
  'natScore': 45,
  'order': 1,
  'subject': 'English'},
 {'grade': 'GC2',
  'index': '18-4',
  'level': 'G',
  'name': '088198ee59',
  'natScore': 88,
  'order': 1,
  'subject': 'Maths'},
 {'grade': 'GC3',
  'index': '18-5',
  'level': 'G',
  'name': '088198ee59',
  'natScore': 99,
  'order': 1,
  'subject': 'History'},
 {'grade': 'GC3',
  'index': '18-9',
  'level': 'G',
  'name': '088198ee59',
  'natScore': 9,
  'order': 1,
  'subject': 'Spanish'},
 {'grade': 'AD3',
  'index': '18-10',
  'level': 'A',
  'name': '088198ee59',
  'natScore': 23,
  'order': 1,
  'subject': 'Art'},
 {'grade': 'GC2',
  'index': '18-15',
  'level': 'G',
  'name': '088198ee59',
  'natScore': 4,
  'order': 1,
  'subject': 'Biology'},
 {'grade': 'GC2',
  'index': '19-2',
  'level': 'G',
  'name': 'fa47672706',
  'natScore': 88,
  'order': 1,
  'subject': 'Irish'},
 {'grade': 'GC1',
  'index': '19-3',
  'level': 'G',
  'name': 'fa47672706',
  'natScore': 7,
  'order': 1,
  'subject': 'English'},
 {'grade': 'AC3',
  'index': '19-4',
  'level': 'A',
  'name': 'fa47672706',
  'natScore': 58,
  'order': 1,
  'subject': 'Maths'},
 {'grade': 'AD1',
  'index': '19-6',
  'level': 'A',
  'name': 'fa47672706',
  'natScore': 55,
  'order': 1,
  'subject': 'Geography'},
 {'grade': 'AC2',
  'index': '19-12',
  'level': 'A',
  'name': 'fa47672706',
  'natScore': 81,
  'order': 1,
  'subject': 'Physics'},
 {'grade': 'AB2',
  'index': '19-16',
  'level': 'A',
  'name': 'fa47672706',
  'natScore': 56,
  'order': 1,
  'subject': 'Engineering'},
 {'grade': 'AB3',
  'index': '19-17',
  'level': 'A',
  'name': 'fa47672706',
  'natScore': 70,
  'order': 1,
  'subject': 'Construction Studies'},
 {'grade': 'AB1',
  'index': '19-24',
  'level': 'A',
  'name': 'fa47672706',
  'natScore': 0,
  'order': 1,
  'subject': 'Design & Comm. Graphics'},
 {'grade': 'GC3',
  'index': '20-2',
  'level': 'G',
  'name': 'e13b0402cf',
  'natScore': 58,
  'order': 1,
  'subject': 'Irish'},
 {'grade': 'AC2',
  'index': '20-3',
  'level': 'A',
  'name': 'e13b0402cf',
  'natScore': 83,
  'order': 1,
  'subject': 'English'},
 {'grade': 'AD1',
  'index': '20-4',
  'level': 'A',
  'name': 'e13b0402cf',
  'natScore': 45,
  'order': 1,
  'subject': 'Maths'},
 {'grade': 'AC2',
  'index': '20-7',
  'level': 'A',
  'name': 'e13b0402cf',
  'natScore': 65,
  'order': 1,
  'subject': 'French'},
 {'grade': 'AC3',
  'index': '20-15',
  'level': 'A',
  'name': 'e13b0402cf',
  'natScore': 15,
  'order': 1,
  'subject': 'Biology'},
 {'grade': 'AB3',
  'index': '20-18',
  'level': 'A',
  'name': 'e13b0402cf',
  'natScore': 95,
  'order': 1,
  'subject': 'Accounting'},
 {'grade': 'AC1',
  'index': '20-19',
  'level': 'A',
  'name': 'e13b0402cf',
  'natScore': 35,
  'order': 1,
  'subject': 'Business'},
 {'grade': 'GC1',
  'index': '21-2',
  'level': 'G',
  'name': '10385d5b65',
  'natScore': 35,
  'order': 1,
  'subject': 'Irish'},
 {'grade': 'AC2',
  'index': '21-3',
  'level': 'A',
  'name': '10385d5b65',
  'natScore': 71,
  'order': 1,
  'subject': 'English'},
 {'grade': 'GB3',
  'index': '21-4',
  'level': 'G',
  'name': '10385d5b65',
  'natScore': 23,
  'order': 1,
  'subject': 'Maths'},
 {'grade': 'AC2',
  'index': '21-6',
  'level': 'A',
  'name': '10385d5b65',
  'natScore': 33,
  'order': 1,
  'subject': 'Geography'},
 {'grade': 'AD2',
  'index': '21-14',
  'level': 'A',
  'name': '10385d5b65',
  'natScore': 57,
  'order': 1,
  'subject': 'Ag. Science'},
 {'grade': 'AC1',
  'index': '21-19',
  'level': 'A',
  'name': '10385d5b65',
  'natScore': 12,
  'order': 1,
  'subject': 'Business'},
 {'grade': 'AC2',
  'index': '21-22',
  'level': 'A',
  'name': '10385d5b65',
  'natScore': 99,
  'order': 1,
  'subject': 'Home Ec.'},
 {'grade': 'GC3',
  'index': '22-2',
  'level': 'G',
  'name': '21108bd47f',
  'natScore': 25,
  'order': 1,
  'subject': 'Irish'},
 {'grade': 'GB3',
  'index': '22-3',
  'level': 'G',
  'name': '21108bd47f',
  'natScore': 68,
  'order': 1,
  'subject': 'English'},
 {'grade': 'GD1',
  'index': '22-4',
  'level': 'G',
  'name': '21108bd47f',
  'natScore': 30,
  'order': 1,
  'subject': 'Maths'},
 {'grade': 'AD2',
  'index': '22-6',
  'level': 'A',
  'name': '21108bd47f',
  'natScore': 99,
  'order': 1,
  'subject': 'Geography'},
 {'grade': 'AD1',
  'index': '22-14',
  'level': 'A',
  'name': '21108bd47f',
  'natScore': 48,
  'order': 1,
  'subject': 'Ag. Science'},
 {'grade': 'AD2',
  'index': '22-15',
  'level': 'A',
  'name': '21108bd47f',
  'natScore': 39,
  'order': 1,
  'subject': 'Biology'},
 {'grade': 'AD1',
  'index': '22-16',
  'level': 'A',
  'name': '21108bd47f',
  'natScore': 36,
  'order': 1,
  'subject': 'Engineering'},
 {'grade': 'AC1',
  'index': '23-2',
  'level': 'A',
  'name': 'c4e302c007',
  'natScore': 27,
  'order': 1,
  'subject': 'Irish'},
 {'grade': 'AC2',
  'index': '23-3',
  'level': 'A',
  'name': 'c4e302c007',
  'natScore': 11,
  'order': 1,
  'subject': 'English'},
 {'grade': 'GC2',
  'index': '23-4',
  'level': 'G',
  'name': 'c4e302c007',
  'natScore': 19,
  'order': 1,
  'subject': 'Maths'},
 {'grade': 'AC1',
  'index': '23-6',
  'level': 'A',
  'name': 'c4e302c007',
  'natScore': 47,
  'order': 1,
  'subject': 'Geography'},
 {'grade': 'AC1',
  'index': '23-8',
  'level': 'A',
  'name': 'c4e302c007',
  'natScore': 4,
  'order': 1,
  'subject': 'German'},
 {'grade': 'AB1',
  'index': '23-10',
  'level': 'A',
  'name': 'c4e302c007',
  'natScore': 19,
  'order': 1,
  'subject': 'Art'},
 {'grade': 'AD1',
  'index': '23-15',
  'level': 'A',
  'name': 'c4e302c007',
  'natScore': 30,
  'order': 1,
  'subject': 'Biology'},
 {'grade': 'GC2',
  'index': '24-2',
  'level': 'G',
  'name': 'fbe9e7f974',
  'natScore': 70,
  'order': 1,
  'subject': 'Irish'},
 {'grade': 'AC2',
  'index': '24-3',
  'level': 'A',
  'name': 'fbe9e7f974',
  'natScore': 1,
  'order': 1,
  'subject': 'English'},
 {'grade': 'GC2',
  'index': '24-4',
  'level': 'G',
  'name': 'fbe9e7f974',
  'natScore': 72,
  'order': 1,
  'subject': 'Maths'},
 {'grade': 'AC3',
  'index': '24-5',
  'level': 'A',
  'name': 'fbe9e7f974',
  'natScore': 36,
  'order': 1,
  'subject': 'History'},
 {'grade': 'AC3',
  'index': '24-6',
  'level': 'A',
  'name': 'fbe9e7f974',
  'natScore': 90,
  'order': 1,
  'subject': 'Geography'},
 {'grade': 'GB3',
  'index': '24-15',
  'level': 'G',
  'name': 'fbe9e7f974',
  'natScore': 63,
  'order': 1,
  'subject': 'Biology'},
 {'grade': 'AC3',
  'index': '25-2',
  'level': 'A',
  'name': 'e0e1bd0a59',
  'natScore': 91,
  'order': 1,
  'subject': 'Irish'},
 {'grade': 'AB3',
  'index': '25-3',
  'level': 'A',
  'name': 'e0e1bd0a59',
  'natScore': 95,
  'order': 1,
  'subject': 'English'},
 {'grade': 'AC2',
  'index': '25-4',
  'level': 'A',
  'name': 'e0e1bd0a59',
  'natScore': 39,
  'order': 1,
  'subject': 'Maths'},
 {'grade': 'AC3',
  'index': '25-6',
  'level': 'A',
  'name': 'e0e1bd0a59',
  'natScore': 27,
  'order': 1,
  'subject': 'Geography'},
 {'grade': 'AB3',
  'index': '25-7',
  'level': 'A',
  'name': 'e0e1bd0a59',
  'natScore': 13,
  'order': 1,
  'subject': 'French'},
 {'grade': 'AB3',
  'index': '25-15',
  'level': 'A',
  'name': 'e0e1bd0a59',
  'natScore': 14,
  'order': 1,
  'subject': 'Biology'},
 {'grade': 'AD3',
  'index': '25-18',
  'level': 'A',
  'name': 'e0e1bd0a59',
  'natScore': 44,
  'order': 1,
  'subject': 'Accounting'},
 {'grade': 'AC1',
  'index': '26-2',
  'level': 'A',
  'name': 'de49a7f7d7',
  'natScore': 43,
  'order': 1,
  'subject': 'Irish'},
 {'grade': 'AC3',
  'index': '26-3',
  'level': 'A',
  'name': 'de49a7f7d7',
  'natScore': 73,
  'order': 1,
  'subject': 'English'},
 {'grade': 'GC1',
  'index': '26-4',
  'level': 'G',
  'name': 'de49a7f7d7',
  'natScore': 67,
  'order': 1,
  'subject': 'Maths'},
 {'grade': 'AB2',
  'index': '26-5',
  'level': 'A',
  'name': 'de49a7f7d7',
  'natScore': 83,
  'order': 1,
  'subject': 'History'},
 {'grade': 'AB2',
  'index': '26-6',
  'level': 'A',
  'name': 'de49a7f7d7',
  'natScore': 2,
  'order': 1,
  'subject': 'Geography'},
 {'grade': 'AD2',
  'index': '26-8',
  'level': 'A',
  'name': 'de49a7f7d7',
  'natScore': 15,
  'order': 1,
  'subject': 'German'},
 {'grade': 'AC2',
  'index': '26-15',
  'level': 'A',
  'name': 'de49a7f7d7',
  'natScore': 43,
  'order': 1,
  'subject': 'Biology'},
 {'grade': 'AC3',
  'index': '27-2',
  'level': 'A',
  'name': '3461f3b91a',
  'natScore': 87,
  'order': 1,
  'subject': 'Irish'},
 {'grade': 'AA2',
  'index': '27-3',
  'level': 'A',
  'name': '3461f3b91a',
  'natScore': 47,
  'order': 1,
  'subject': 'English'},
 {'grade': 'AA2',
  'index': '27-4',
  'level': 'A',
  'name': '3461f3b91a',
  'natScore': 20,
  'order': 1,
  'subject': 'Maths'},
 {'grade': 'AA1',
  'index': '27-7',
  'level': 'A',
  'name': '3461f3b91a',
  'natScore': 83,
  'order': 1,
  'subject': 'French'},
 {'grade': 'AA1',
  'index': '27-14',
  'level': 'A',
  'name': '3461f3b91a',
  'natScore': 76,
  'order': 1,
  'subject': 'Ag. Science'},
 {'grade': 'AA1',
  'index': '27-15',
  'level': 'A',
  'name': '3461f3b91a',
  'natScore': 90,
  'order': 1,
  'subject': 'Biology'},
 {'grade': 'AB1',
  'index': '27-18',
  'level': 'A',
  'name': '3461f3b91a',
  'natScore': 47,
  'order': 1,
  'subject': 'Accounting'},
 {'grade': 'AC1',
  'index': '28-2',
  'level': 'A',
  'name': 'a160ee0267',
  'natScore': 25,
  'order': 1,
  'subject': 'Irish'},
 {'grade': 'AB3',
  'index': '28-3',
  'level': 'A',
  'name': 'a160ee0267',
  'natScore': 46,
  'order': 1,
  'subject': 'English'},
 {'grade': 'GC1',
  'index': '28-4',
  'level': 'G',
  'name': 'a160ee0267',
  'natScore': 58,
  'order': 1,
  'subject': 'Maths'},
 {'grade': 'AC1',
  'index': '28-8',
  'level': 'A',
  'name': 'a160ee0267',
  'natScore': 32,
  'order': 1,
  'subject': 'German'},
 {'grade': 'AC3',
  'index': '28-15',
  'level': 'A',
  'name': 'a160ee0267',
  'natScore': 4,
  'order': 1,
  'subject': 'Biology'},
 {'grade': 'AB3',
  'index': '28-21',
  'level': 'A',
  'name': 'a160ee0267',
  'natScore': 69,
  'order': 1,
  'subject': 'Music'},
 {'grade': 'AB3',
  'index': '28-22',
  'level': 'A',
  'name': 'a160ee0267',
  'natScore': 22,
  'order': 1,
  'subject': 'Home Ec.'},
 {'grade': 'AB2',
  'index': '29-2',
  'level': 'A',
  'name': 'f4b7a63bc0',
  'natScore': 0,
  'order': 1,
  'subject': 'Irish'},
 {'grade': 'AB1',
  'index': '29-3',
  'level': 'A',
  'name': 'f4b7a63bc0',
  'natScore': 86,
  'order': 1,
  'subject': 'English'},
 {'grade': 'AC2',
  'index': '29-4',
  'level': 'A',
  'name': 'f4b7a63bc0',
  'natScore': 46,
  'order': 1,
  'subject': 'Maths'},
 {'grade': 'GB1',
  'index': '29-9',
  'level': 'G',
  'name': 'f4b7a63bc0',
  'natScore': 42,
  'order': 1,
  'subject': 'Spanish'},
 {'grade': 'AC2',
  'index': '29-13',
  'level': 'A',
  'name': 'f4b7a63bc0',
  'natScore': 4,
  'order': 1,
  'subject': 'Chemistry'},
 {'grade': 'AB2',
  'index': '29-15',
  'level': 'A',
  'name': 'f4b7a63bc0',
  'natScore': 32,
  'order': 1,
  'subject': 'Biology'},
 {'grade': 'AC1',
  'index': '29-20',
  'level': 'A',
  'name': 'f4b7a63bc0',
  'natScore': 55,
  'order': 1,
  'subject': 'Economics'},
 {'grade': 'GC2',
  'index': '30-2',
  'level': 'G',
  'name': 'bbdb3fd6fa',
  'natScore': 26,
  'order': 1,
  'subject': 'Irish'},
 {'grade': 'AD1',
  'index': '30-3',
  'level': 'A',
  'name': 'bbdb3fd6fa',
  'natScore': 44,
  'order': 1,
  'subject': 'English'},
 {'grade': 'AC3',
  'index': '30-4',
  'level': 'A',
  'name': 'bbdb3fd6fa',
  'natScore': 64,
  'order': 1,
  'subject': 'Maths'},
 {'grade': 'AC3',
  'index': '30-6',
  'level': 'A',
  'name': 'bbdb3fd6fa',
  'natScore': 25,
  'order': 1,
  'subject': 'Geography'},
 {'grade': 'AD2',
  'index': '30-12',
  'level': 'A',
  'name': 'bbdb3fd6fa',
  'natScore': 82,
  'order': 1,
  'subject': 'Physics'},
 {'grade': 'AB3',
  'index': '30-16',
  'level': 'A',
  'name': 'bbdb3fd6fa',
  'natScore': 84,
  'order': 1,
  'subject': 'Engineering'},
 {'grade': 'AB2',
  'index': '30-17',
  'level': 'A',
  'name': 'bbdb3fd6fa',
  'natScore': 27,
  'order': 1,
  'subject': 'Construction Studies'},
 {'grade': 'AB2',
  'index': '31-2',
  'level': 'A',
  'name': '10ffb2cb6f',
  'natScore': 85,
  'order': 1,
  'subject': 'Irish'},
 {'grade': 'AD1',
  'index': '31-3',
  'level': 'A',
  'name': '10ffb2cb6f',
  'natScore': 10,
  'order': 1,
  'subject': 'English'},
 {'grade': 'GB1',
  'index': '31-4',
  'level': 'G',
  'name': '10ffb2cb6f',
  'natScore': 13,
  'order': 1,
  'subject': 'Maths'},
 {'grade': 'AB1',
  'index': '31-8',
  'level': 'A',
  'name': '10ffb2cb6f',
  'natScore': 52,
  'order': 1,
  'subject': 'German'},
 {'grade': 'AB3',
  'index': '31-13',
  'level': 'A',
  'name': '10ffb2cb6f',
  'natScore': 77,
  'order': 1,
  'subject': 'Chemistry'},
 {'grade': 'AA2',
  'index': '31-15',
  'level': 'A',
  'name': '10ffb2cb6f',
  'natScore': 27,
  'order': 1,
  'subject': 'Biology'},
 {'grade': 'AB2',
  'index': '31-22',
  'level': 'A',
  'name': '10ffb2cb6f',
  'natScore': 7,
  'order': 1,
  'subject': 'Home Ec.'},
 {'grade': 'GC2',
  'index': '32-2',
  'level': 'G',
  'name': '51c0b1e960',
  'natScore': 3,
  'order': 1,
  'subject': 'Irish'},
 {'grade': 'AD3',
  'index': '32-3',
  'level': 'A',
  'name': '51c0b1e960',
  'natScore': 40,
  'order': 1,
  'subject': 'English'},
 {'grade': 'GD3',
  'index': '32-4',
  'level': 'G',
  'name': '51c0b1e960',
  'natScore': 21,
  'order': 1,
  'subject': 'Maths'},
 {'grade': 'AD1',
  'index': '32-6',
  'level': 'A',
  'name': '51c0b1e960',
  'natScore': 51,
  'order': 1,
  'subject': 'Geography'},
 {'grade': 'AC3',
  'index': '32-14',
  'level': 'A',
  'name': '51c0b1e960',
  'natScore': 86,
  'order': 1,
  'subject': 'Ag. Science'},
 {'grade': 'AC1',
  'index': '32-15',
  'level': 'A',
  'name': '51c0b1e960',
  'natScore': 54,
  'order': 1,
  'subject': 'Biology'},
 {'grade': 'AC3',
  'index': '32-16',
  'level': 'A',
  'name': '51c0b1e960',
  'natScore': 72,
  'order': 1,
  'subject': 'Engineering'},
 {'grade': 'AC2',
  'index': '33-2',
  'level': 'A',
  'name': 'd67e2fcbf3',
  'natScore': 33,
  'order': 1,
  'subject': 'Irish'},
 {'grade': 'AD3',
  'index': '33-3',
  'level': 'A',
  'name': 'd67e2fcbf3',
  'natScore': 12,
  'order': 1,
  'subject': 'English'},
 {'grade': 'AE',
  'index': '33-4',
  'level': 'A',
  'name': 'd67e2fcbf3',
  'natScore': 70,
  'order': 1,
  'subject': 'Maths'},
 {'grade': 'AD2',
  'index': '33-6',
  'level': 'A',
  'name': 'd67e2fcbf3',
  'natScore': 87,
  'order': 1,
  'subject': 'Geography'},
 {'grade': 'AF',
  'index': '33-12',
  'level': 'A',
  'name': 'd67e2fcbf3',
  'natScore': 54,
  'order': 1,
  'subject': 'Physics'},
 {'grade': 'AC2',
  'index': '33-14',
  'level': 'A',
  'name': 'd67e2fcbf3',
  'natScore': 12,
  'order': 1,
  'subject': 'Ag. Science'},
 {'grade': 'AB3',
  'index': '33-15',
  'level': 'A',
  'name': 'd67e2fcbf3',
  'natScore': 7,
  'order': 1,
  'subject': 'Biology'},
 {'grade': 'AD3',
  'index': '33-19',
  'level': 'A',
  'name': 'd67e2fcbf3',
  'natScore': 36,
  'order': 1,
  'subject': 'Business'},
 {'grade': 'AC3',
  'index': '34-2',
  'level': 'A',
  'name': '4ed4302724',
  'natScore': 56,
  'order': 1,
  'subject': 'Irish'},
 {'grade': 'AB2',
  'index': '34-3',
  'level': 'A',
  'name': '4ed4302724',
  'natScore': 96,
  'order': 1,
  'subject': 'English'},
 {'grade': 'AC2',
  'index': '34-4',
  'level': 'A',
  'name': '4ed4302724',
  'natScore': 8,
  'order': 1,
  'subject': 'Maths'},
 {'grade': 'GB3',
  'index': '34-7',
  'level': 'G',
  'name': '4ed4302724',
  'natScore': 61,
  'order': 1,
  'subject': 'French'},
 {'grade': 'AD1',
  'index': '34-15',
  'level': 'A',
  'name': '4ed4302724',
  'natScore': 37,
  'order': 1,
  'subject': 'Biology'},
 {'grade': 'AB3',
  'index': '34-18',
  'level': 'A',
  'name': '4ed4302724',
  'natScore': 65,
  'order': 1,
  'subject': 'Accounting'},
 {'grade': 'AB3',
  'index': '34-19',
  'level': 'A',
  'name': '4ed4302724',
  'natScore': 92,
  'order': 1,
  'subject': 'Business'},
 {'grade': 'GC1',
  'index': '35-2',
  'level': 'G',
  'name': 'eb7fa684a7',
  'natScore': 10,
  'order': 1,
  'subject': 'Irish'},
 {'grade': 'GB1',
  'index': '35-3',
  'level': 'G',
  'name': 'eb7fa684a7',
  'natScore': 89,
  'order': 1,
  'subject': 'English'},
 {'grade': 'AD2',
  'index': '35-4',
  'level': 'A',
  'name': 'eb7fa684a7',
  'natScore': 24,
  'order': 1,
  'subject': 'Maths'},
 {'grade': 'AB3',
  'index': '35-6',
  'level': 'A',
  'name': 'eb7fa684a7',
  'natScore': 43,
  'order': 1,
  'subject': 'Geography'},
 {'grade': 'AC2',
  'index': '35-13',
  'level': 'A',
  'name': 'eb7fa684a7',
  'natScore': 48,
  'order': 1,
  'subject': 'Chemistry'},
 {'grade': 'AC1',
  'index': '35-14',
  'level': 'A',
  'name': 'eb7fa684a7',
  'natScore': 100,
  'order': 1,
  'subject': 'Ag. Science'},
 {'grade': 'AC1',
  'index': '35-15',
  'level': 'A',
  'name': 'eb7fa684a7',
  'natScore': 1,
  'order': 1,
  'subject': 'Biology'},
 {'grade': 'AC2',
  'index': '35-20',
  'level': 'A',
  'name': 'eb7fa684a7',
  'natScore': 9,
  'order': 1,
  'subject': 'Economics'},
 {'grade': 'GC1',
  'index': '36-2',
  'level': 'G',
  'name': '5190d80f1e',
  'natScore': 21,
  'order': 1,
  'subject': 'Irish'},
 {'grade': 'GA2',
  'index': '36-3',
  'level': 'G',
  'name': '5190d80f1e',
  'natScore': 83,
  'order': 1,
  'subject': 'English'},
 {'grade': 'GB1',
  'index': '36-4',
  'level': 'G',
  'name': '5190d80f1e',
  'natScore': 23,
  'order': 1,
  'subject': 'Maths'},
 {'grade': 'AC3',
  'index': '36-6',
  'level': 'A',
  'name': '5190d80f1e',
  'natScore': 71,
  'order': 1,
  'subject': 'Geography'},
 {'grade': 'AD2',
  'index': '36-10',
  'level': 'A',
  'name': '5190d80f1e',
  'natScore': 58,
  'order': 1,
  'subject': 'Art'},
 {'grade': 'AB2',
  'index': '36-16',
  'level': 'A',
  'name': '5190d80f1e',
  'natScore': 40,
  'order': 1,
  'subject': 'Engineering'},
 {'grade': 'AB3',
  'index': '36-17',
  'level': 'A',
  'name': '5190d80f1e',
  'natScore': 61,
  'order': 1,
  'subject': 'Construction Studies'},
 {'grade': 'GC2',
  'index': '37-2',
  'level': 'G',
  'name': '548f4fa9cb',
  'natScore': 40,
  'order': 1,
  'subject': 'Irish'},
 {'grade': 'AC2',
  'index': '37-3',
  'level': 'A',
  'name': '548f4fa9cb',
  'natScore': 40,
  'order': 1,
  'subject': 'English'},
 {'grade': 'GC1',
  'index': '37-4',
  'level': 'G',
  'name': '548f4fa9cb',
  'natScore': 52,
  'order': 1,
  'subject': 'Maths'},
 {'grade': 'AC2',
  'index': '37-6',
  'level': 'A',
  'name': '548f4fa9cb',
  'natScore': 15,
  'order': 1,
  'subject': 'Geography'},
 {'grade': 'AC3',
  'index': '37-15',
  'level': 'A',
  'name': '548f4fa9cb',
  'natScore': 21,
  'order': 1,
  'subject': 'Biology'},
 {'grade': 'AB3',
  'index': '37-16',
  'level': 'A',
  'name': '548f4fa9cb',
  'natScore': 51,
  'order': 1,
  'subject': 'Engineering'},
 {'grade': 'GE',
  'index': '37-18',
  'level': 'G',
  'name': '548f4fa9cb',
  'natScore': 61,
  'order': 1,
  'subject': 'Accounting'},
 {'grade': 'GC2',
  'index': '38-2',
  'level': 'G',
  'name': '5d75758f1a',
  'natScore': 70,
  'order': 1,
  'subject': 'Irish'},
 {'grade': 'AC3',
  'index': '38-3',
  'level': 'A',
  'name': '5d75758f1a',
  'natScore': 78,
  'order': 1,
  'subject': 'English'},
 {'grade': 'GC1',
  'index': '38-4',
  'level': 'G',
  'name': '5d75758f1a',
  'natScore': 49,
  'order': 1,
  'subject': 'Maths'},
 {'grade': 'AD1',
  'index': '38-6',
  'level': 'A',
  'name': '5d75758f1a',
  'natScore': 2,
  'order': 1,
  'subject': 'Geography'},
 {'grade': 'ANG',
  'index': '38-13',
  'level': 'A',
  'name': '5d75758f1a',
  'natScore': 74,
  'order': 1,
  'subject': 'Chemistry'},
 {'grade': 'AD1',
  'index': '38-15',
  'level': 'A',
  'name': '5d75758f1a',
  'natScore': 62,
  'order': 1,
  'subject': 'Biology'},
 {'grade': 'AF',
  'index': '38-20',
  'level': 'A',
  'name': '5d75758f1a',
  'natScore': 72,
  'order': 1,
  'subject': 'Economics'},
 {'grade': 'AC3',
  'index': '39-2',
  'level': 'A',
  'name': 'f345e853a5',
  'natScore': 31,
  'order': 1,
  'subject': 'Irish'},
 {'grade': 'AB3',
  'index': '39-3',
  'level': 'A',
  'name': 'f345e853a5',
  'natScore': 22,
  'order': 1,
  'subject': 'English'},
 {'grade': 'GB2',
  'index': '39-4',
  'level': 'G',
  'name': 'f345e853a5',
  'natScore': 17,
  'order': 1,
  'subject': 'Maths'},
 {'grade': 'AB3',
  'index': '39-6',
  'level': 'A',
  'name': 'f345e853a5',
  'natScore': 67,
  'order': 1,
  'subject': 'Geography'},
 {'grade': 'AC1',
  'index': '39-7',
  'level': 'A',
  'name': 'f345e853a5',
  'natScore': 8,
  'order': 1,
  'subject': 'French'},
 {'grade': 'AB2',
  'index': '39-15',
  'level': 'A',
  'name': 'f345e853a5',
  'natScore': 20,
  'order': 1,
  'subject': 'Biology'},
 {'grade': 'AB1',
  'index': '39-19',
  'level': 'A',
  'name': 'f345e853a5',
  'natScore': 16,
  'order': 1,
  'subject': 'Business'},
 {'grade': 'GF',
  'index': '40-2',
  'level': 'G',
  'name': '4818badca1',
  'natScore': 50,
  'order': 1,
  'subject': 'Irish'},
 {'grade': 'AD1',
  'index': '40-3',
  'level': 'A',
  'name': '4818badca1',
  'natScore': 0,
  'order': 1,
  'subject': 'English'},
 {'grade': 'AC3',
  'index': '40-4',
  'level': 'A',
  'name': '4818badca1',
  'natScore': 82,
  'order': 1,
  'subject': 'Maths'},
 {'grade': 'AC1',
  'index': '40-6',
  'level': 'A',
  'name': '4818badca1',
  'natScore': 99,
  'order': 1,
  'subject': 'Geography'},
 {'grade': 'AC1',
  'index': '40-10',
  'level': 'A',
  'name': '4818badca1',
  'natScore': 21,
  'order': 1,
  'subject': 'Art'},
 {'grade': 'AA1',
  'index': '40-14',
  'level': 'A',
  'name': '4818badca1',
  'natScore': 44,
  'order': 1,
  'subject': 'Ag. Science'},
 {'grade': 'AB2',
  'index': '40-15',
  'level': 'A',
  'name': '4818badca1',
  'natScore': 55,
  'order': 1,
  'subject': 'Biology'},
 {'grade': 'AB3',
  'index': '41-2',
  'level': 'A',
  'name': 'a1ab4c0be2',
  'natScore': 76,
  'order': 1,
  'subject': 'Irish'},
 {'grade': 'AD1',
  'index': '41-3',
  'level': 'A',
  'name': 'a1ab4c0be2',
  'natScore': 50,
  'order': 1,
  'subject': 'English'},
 {'grade': 'AC2',
  'index': '41-5',
  'level': 'A',
  'name': 'a1ab4c0be2',
  'natScore': 25,
  'order': 1,
  'subject': 'History'},
 {'grade': 'AB3',
  'index': '41-6',
  'level': 'A',
  'name': 'a1ab4c0be2',
  'natScore': 43,
  'order': 1,
  'subject': 'Geography'},
 {'grade': 'AD1',
  'index': '41-10',
  'level': 'A',
  'name': 'a1ab4c0be2',
  'natScore': 20,
  'order': 1,
  'subject': 'Art'},
 {'grade': 'AB3',
  'index': '41-15',
  'level': 'A',
  'name': 'a1ab4c0be2',
  'natScore': 51,
  'order': 1,
  'subject': 'Biology'},
 {'grade': 'GC2',
  'index': '42-3',
  'level': 'G',
  'name': '2c57d7f955',
  'natScore': 17,
  'order': 1,
  'subject': 'English'},
 {'grade': 'GD3',
  'index': '42-5',
  'level': 'G',
  'name': '2c57d7f955',
  'natScore': 24,
  'order': 1,
  'subject': 'History'},
 {'grade': 'GC3',
  'index': '42-6',
  'level': 'G',
  'name': '2c57d7f955',
  'natScore': 79,
  'order': 1,
  'subject': 'Geography'},
 {'grade': 'GD3',
  'index': '42-10',
  'level': 'G',
  'name': '2c57d7f955',
  'natScore': 26,
  'order': 1,
  'subject': 'Art'},
 {'grade': 'GE',
  'index': '42-22',
  'level': 'G',
  'name': '2c57d7f955',
  'natScore': 5,
  'order': 1,
  'subject': 'Home Ec.'},
 {'grade': 'GA1',
  'index': '43-3',
  'level': 'G',
  'name': '4b513c98ea',
  'natScore': 36,
  'order': 1,
  'subject': 'English'},
 {'grade': 'GA2',
  'index': '43-4',
  'level': 'G',
  'name': '4b513c98ea',
  'natScore': 41,
  'order': 1,
  'subject': 'Maths'},
 {'grade': 'AC1',
  'index': '43-6',
  'level': 'A',
  'name': '4b513c98ea',
  'natScore': 38,
  'order': 1,
  'subject': 'Geography'},
 {'grade': 'AB2',
  'index': '43-10',
  'level': 'A',
  'name': '4b513c98ea',
  'natScore': 48,
  'order': 1,
  'subject': 'Art'},
 {'grade': 'AC2',
  'index': '43-14',
  'level': 'A',
  'name': '4b513c98ea',
  'natScore': 85,
  'order': 1,
  'subject': 'Ag. Science'},
 {'grade': 'AB2',
  'index': '43-16',
  'level': 'A',
  'name': '4b513c98ea',
  'natScore': 70,
  'order': 1,
  'subject': 'Engineering'},
 {'grade': 'AC1',
  'index': '43-22',
  'level': 'A',
  'name': '4b513c98ea',
  'natScore': 98,
  'order': 1,
  'subject': 'Home Ec.'},
 {'grade': 'GC1',
  'index': '44-2',
  'level': 'G',
  'name': '21c5051238',
  'natScore': 82,
  'order': 1,
  'subject': 'Irish'},
 {'grade': 'AC3',
  'index': '44-3',
  'level': 'A',
  'name': '21c5051238',
  'natScore': 29,
  'order': 1,
  'subject': 'English'},
 {'grade': 'GB3',
  'index': '44-4',
  'level': 'G',
  'name': '21c5051238',
  'natScore': 20,
  'order': 1,
  'subject': 'Maths'},
 {'grade': 'AD1',
  'index': '44-9',
  'level': 'A',
  'name': '21c5051238',
  'natScore': 22,
  'order': 1,
  'subject': 'Spanish'},
 {'grade': 'AC2',
  'index': '44-14',
  'level': 'A',
  'name': '21c5051238',
  'natScore': 79,
  'order': 1,
  'subject': 'Ag. Science'},
 {'grade': 'AC1',
  'index': '44-19',
  'level': 'A',
  'name': '21c5051238',
  'natScore': 39,
  'order': 1,
  'subject': 'Business'},
 {'grade': 'AC1',
  'index': '44-22',
  'level': 'A',
  'name': '21c5051238',
  'natScore': 93,
  'order': 1,
  'subject': 'Home Ec.'},
 {'grade': 'AB1',
  'index': '45-3',
  'level': 'A',
  'name': '2499e50fa4',
  'natScore': 6,
  'order': 1,
  'subject': 'English'},
 {'grade': 'GA2',
  'index': '45-4',
  'level': 'G',
  'name': '2499e50fa4',
  'natScore': 63,
  'order': 1,
  'subject': 'Maths'},
 {'grade': 'AB2',
  'index': '45-8',
  'level': 'A',
  'name': '2499e50fa4',
  'natScore': 66,
  'order': 1,
  'subject': 'German'},
 {'grade': 'AB2',
  'index': '45-13',
  'level': 'A',
  'name': '2499e50fa4',
  'natScore': 51,
  'order': 1,
  'subject': 'Chemistry'},
 {'grade': 'AA1',
  'index': '45-14',
  'level': 'A',
  'name': '2499e50fa4',
  'natScore': 62,
  'order': 1,
  'subject': 'Ag. Science'},
 {'grade': 'AA1',
  'index': '45-15',
  'level': 'A',
  'name': '2499e50fa4',
  'natScore': 52,
  'order': 1,
  'subject': 'Biology'},
 {'grade': 'AA2',
  'index': '45-20',
  'level': 'A',
  'name': '2499e50fa4',
  'natScore': 23,
  'order': 1,
  'subject': 'Economics'},
 {'grade': 'GB3',
  'index': '46-2',
  'level': 'G',
  'name': '2523a69417',
  'natScore': 8,
  'order': 1,
  'subject': 'Irish'},
 {'grade': 'GA2',
  'index': '46-3',
  'level': 'G',
  'name': '2523a69417',
  'natScore': 80,
  'order': 1,
  'subject': 'English'},
 {'grade': 'AC2',
  'index': '46-4',
  'level': 'A',
  'name': '2523a69417',
  'natScore': 61,
  'order': 1,
  'subject': 'Maths'},
 {'grade': 'AC1',
  'index': '46-6',
  'level': 'A',
  'name': '2523a69417',
  'natScore': 13,
  'order': 1,
  'subject': 'Geography'},
 {'grade': 'AC3',
  'index': '46-9',
  'level': 'A',
  'name': '2523a69417',
  'natScore': 25,
  'order': 1,
  'subject': 'Spanish'},
 {'grade': 'AC3',
  'index': '46-14',
  'level': 'A',
  'name': '2523a69417',
  'natScore': 60,
  'order': 1,
  'subject': 'Ag. Science'},
 {'grade': 'AB3',
  'index': '46-19',
  'level': 'A',
  'name': '2523a69417',
  'natScore': 86,
  'order': 1,
  'subject': 'Business'},
 {'grade': 'AC2',
  'index': '47-2',
  'level': 'A',
  'name': 'e9c114e172',
  'natScore': 16,
  'order': 1,
  'subject': 'Irish'},
 {'grade': 'AC2',
  'index': '47-3',
  'level': 'A',
  'name': 'e9c114e172',
  'natScore': 76,
  'order': 1,
  'subject': 'English'},
 {'grade': 'AD1',
  'index': '47-4',
  'level': 'A',
  'name': 'e9c114e172',
  'natScore': 89,
  'order': 1,
  'subject': 'Maths'},
 {'grade': 'AC1',
  'index': '47-5',
  'level': 'A',
  'name': 'e9c114e172',
  'natScore': 50,
  'order': 1,
  'subject': 'History'},
 {'grade': 'AB2',
  'index': '47-6',
  'level': 'A',
  'name': 'e9c114e172',
  'natScore': 92,
  'order': 1,
  'subject': 'Geography'},
 {'grade': 'AB2',
  'index': '47-7',
  'level': 'A',
  'name': 'e9c114e172',
  'natScore': 10,
  'order': 1,
  'subject': 'French'},
 {'grade': 'AB3',
  'index': '47-19',
  'level': 'A',
  'name': 'e9c114e172',
  'natScore': 46,
  'order': 1,
  'subject': 'Business'},
 {'grade': 'AB3',
  'index': '48-2',
  'level': 'A',
  'name': 'd59ab0e83a',
  'natScore': 39,
  'order': 1,
  'subject': 'Irish'},
 {'grade': 'AB3',
  'index': '48-3',
  'level': 'A',
  'name': 'd59ab0e83a',
  'natScore': 22,
  'order': 1,
  'subject': 'English'},
 {'grade': 'GC3',
  'index': '48-4',
  'level': 'G',
  'name': 'd59ab0e83a',
  'natScore': 94,
  'order': 1,
  'subject': 'Maths'},
 {'grade': 'AC3',
  'index': '48-5',
  'level': 'A',
  'name': 'd59ab0e83a',
  'natScore': 98,
  'order': 1,
  'subject': 'History'},
 {'grade': 'AC3',
  'index': '48-7',
  'level': 'A',
  'name': 'd59ab0e83a',
  'natScore': 29,
  'order': 1,
  'subject': 'French'},
 {'grade': 'AC2',
  'index': '48-15',
  'level': 'A',
  'name': 'd59ab0e83a',
  'natScore': 97,
  'order': 1,
  'subject': 'Biology'},
 {'grade': 'AB2',
  'index': '48-21',
  'level': 'A',
  'name': 'd59ab0e83a',
  'natScore': 23,
  'order': 1,
  'subject': 'Music'},
 {'grade': 'GC3',
  'index': '49-2',
  'level': 'G',
  'name': '7a521ca660',
  'natScore': 50,
  'order': 1,
  'subject': 'Irish'},
 {'grade': 'GB3',
  'index': '49-3',
  'level': 'G',
  'name': '7a521ca660',
  'natScore': 39,
  'order': 1,
  'subject': 'English'},
 {'grade': 'AC3',
  'index': '49-4',
  'level': 'A',
  'name': '7a521ca660',
  'natScore': 44,
  'order': 1,
  'subject': 'Maths'},
 {'grade': 'AC2',
  'index': '49-6',
  'level': 'A',
  'name': '7a521ca660',
  'natScore': 97,
  'order': 1,
  'subject': 'Geography'},
 {'grade': 'AC2',
  'index': '49-12',
  'level': 'A',
  'name': '7a521ca660',
  'natScore': 38,
  'order': 1,
  'subject': 'Physics'},
 {'grade': 'AB2',
  'index': '49-16',
  'level': 'A',
  'name': '7a521ca660',
  'natScore': 37,
  'order': 1,
  'subject': 'Engineering'},
 {'grade': 'AC2',
  'index': '49-17',
  'level': 'A',
  'name': '7a521ca660',
  'natScore': 5,
  'order': 1,
  'subject': 'Construction Studies'},
 {'grade': 'GC2',
  'index': '50-2',
  'level': 'G',
  'name': '462c4f34d9',
  'natScore': 47,
  'order': 1,
  'subject': 'Irish'},
 {'grade': 'AD2',
  'index': '50-3',
  'level': 'A',
  'name': '462c4f34d9',
  'natScore': 36,
  'order': 1,
  'subject': 'English'},
 {'grade': 'AD2',
  'index': '50-4',
  'level': 'A',
  'name': '462c4f34d9',
  'natScore': 28,
  'order': 1,
  'subject': 'Maths'},
 {'grade': 'AC1',
  'index': '50-6',
  'level': 'A',
  'name': '462c4f34d9',
  'natScore': 24,
  'order': 1,
  'subject': 'Geography'},
 {'grade': 'AB3',
  'index': '50-15',
  'level': 'A',
  'name': '462c4f34d9',
  'natScore': 47,
  'order': 1,
  'subject': 'Biology'},
 {'grade': 'AC1',
  'index': '50-17',
  'level': 'A',
  'name': '462c4f34d9',
  'natScore': 86,
  'order': 1,
  'subject': 'Construction Studies'},
 {'grade': 'AC2',
  'index': '50-19',
  'level': 'A',
  'name': '462c4f34d9',
  'natScore': 58,
  'order': 1,
  'subject': 'Business'},
 {'grade': 'GD1',
  'index': '51-2',
  'level': 'G',
  'name': '1967b5d985',
  'natScore': 23,
  'order': 1,
  'subject': 'Irish'},
 {'grade': 'AB2',
  'index': '51-3',
  'level': 'A',
  'name': '1967b5d985',
  'natScore': 36,
  'order': 1,
  'subject': 'English'},
 {'grade': 'GC1',
  'index': '51-4',
  'level': 'G',
  'name': '1967b5d985',
  'natScore': 66,
  'order': 1,
  'subject': 'Maths'},
 {'grade': 'AB3',
  'index': '51-6',
  'level': 'A',
  'name': '1967b5d985',
  'natScore': 52,
  'order': 1,
  'subject': 'Geography'},
 {'grade': 'AE',
  'index': '51-13',
  'level': 'A',
  'name': '1967b5d985',
  'natScore': 78,
  'order': 1,
  'subject': 'Chemistry'},
 {'grade': 'AB3',
  'index': '51-15',
  'level': 'A',
  'name': '1967b5d985',
  'natScore': 65,
  'order': 1,
  'subject': 'Biology'},
 {'grade': 'AC1',
  'index': '51-20',
  'level': 'A',
  'name': '1967b5d985',
  'natScore': 41,
  'order': 1,
  'subject': 'Economics'},
 {'grade': 'AB3',
  'index': '52-2',
  'level': 'A',
  'name': '19921bb380',
  'natScore': 66,
  'order': 1,
  'subject': 'Irish'},
 {'grade': 'AA2',
  'index': '52-3',
  'level': 'A',
  'name': '19921bb380',
  'natScore': 35,
  'order': 1,
  'subject': 'English'},
 {'grade': 'AB2',
  'index': '52-4',
  'level': 'A',
  'name': '19921bb380',
  'natScore': 86,
  'order': 1,
  'subject': 'Maths'},
 {'grade': 'AB1',
  'index': '52-9',
  'level': 'A',
  'name': '19921bb380',
  'natScore': 93,
  'order': 1,
  'subject': 'Spanish'},
 {'grade': 'AB3',
  'index': '52-12',
  'level': 'A',
  'name': '19921bb380',
  'natScore': 86,
  'order': 1,
  'subject': 'Physics'},
 {'grade': 'AA1',
  'index': '52-15',
  'level': 'A',
  'name': '19921bb380',
  'natScore': 71,
  'order': 1,
  'subject': 'Biology'},
 {'grade': 'AA2',
  'index': '52-21',
  'level': 'A',
  'name': '19921bb380',
  'natScore': 31,
  'order': 1,
  'subject': 'Music'},
 {'grade': 'AD1',
  'index': '53-2',
  'level': 'A',
  'name': '272b1147c6',
  'natScore': 78,
  'order': 1,
  'subject': 'Irish'},
 {'grade': 'GB1',
  'index': '53-3',
  'level': 'G',
  'name': '272b1147c6',
  'natScore': 74,
  'order': 1,
  'subject': 'English'},
 {'grade': 'GB3',
  'index': '53-4',
  'level': 'G',
  'name': '272b1147c6',
  'natScore': 65,
  'order': 1,
  'subject': 'Maths'},
 {'grade': 'GB3',
  'index': '53-8',
  'level': 'G',
  'name': '272b1147c6',
  'natScore': 35,
  'order': 1,
  'subject': 'German'},
 {'grade': 'AC3',
  'index': '53-14',
  'level': 'A',
  'name': '272b1147c6',
  'natScore': 21,
  'order': 1,
  'subject': 'Ag. Science'},
 {'grade': 'AD2',
  'index': '53-15',
  'level': 'A',
  'name': '272b1147c6',
  'natScore': 26,
  'order': 1,
  'subject': 'Biology'},
 {'grade': 'AD3',
  'index': '53-19',
  'level': 'A',
  'name': '272b1147c6',
  'natScore': 1,
  'order': 1,
  'subject': 'Business'},
 {'grade': 'GC2',
  'index': '54-2',
  'level': 'G',
  'name': 'e69f1c8cd0',
  'natScore': 45,
  'order': 1,
  'subject': 'Irish'},
 {'grade': 'GB3',
  'index': '54-3',
  'level': 'G',
  'name': 'e69f1c8cd0',
  'natScore': 70,
  'order': 1,
  'subject': 'English'},
 {'grade': 'GD1',
  'index': '54-4',
  'level': 'G',
  'name': 'e69f1c8cd0',
  'natScore': 84,
  'order': 1,
  'subject': 'Maths'},
 {'grade': 'AD2',
  'index': '54-6',
  'level': 'A',
  'name': 'e69f1c8cd0',
  'natScore': 90,
  'order': 1,
  'subject': 'Geography'},
 {'grade': 'AE',
  'index': '54-14',
  'level': 'A',
  'name': 'e69f1c8cd0',
  'natScore': 50,
  'order': 1,
  'subject': 'Ag. Science'},
 {'grade': 'AD2',
  'index': '54-19',
  'level': 'A',
  'name': 'e69f1c8cd0',
  'natScore': 89,
  'order': 1,
  'subject': 'Business'},
 {'grade': 'GB2',
  'index': '54-20',
  'level': 'G',
  'name': 'e69f1c8cd0',
  'natScore': 46,
  'order': 1,
  'subject': 'Economics'},
 {'grade': 'AC1',
  'index': '55-2',
  'level': 'A',
  'name': '8ec79898f7',
  'natScore': 22,
  'order': 1,
  'subject': 'Irish'},
 {'grade': 'AB2',
  'index': '55-3',
  'level': 'A',
  'name': '8ec79898f7',
  'natScore': 3,
  'order': 1,
  'subject': 'English'},
 {'grade': 'GA2',
  'index': '55-4',
  'level': 'G',
  'name': '8ec79898f7',
  'natScore': 95,
  'order': 1,
  'subject': 'Maths'},
 {'grade': 'AB3',
  'index': '55-9',
  'level': 'A',
  'name': '8ec79898f7',
  'natScore': 86,
  'order': 1,
  'subject': 'Spanish'},
 {'grade': 'AC3',
  'index': '55-10',
  'level': 'A',
  'name': '8ec79898f7',
  'natScore': 60,
  'order': 1,
  'subject': 'Art'},
 {'grade': 'AD2',
  'index': '55-14',
  'level': 'A',
  'name': '8ec79898f7',
  'natScore': 55,
  'order': 1,
  'subject': 'Ag. Science'},
 {'grade': 'AC2',
  'index': '55-22',
  'level': 'A',
  'name': '8ec79898f7',
  'natScore': 80,
  'order': 1,
  'subject': 'Home Ec.'},
 {'grade': 'GB2',
  'index': '56-2',
  'level': 'G',
  'name': '758ca7d590',
  'natScore': 5,
  'order': 1,
  'subject': 'Irish'},
 {'grade': 'AC1',
  'index': '56-3',
  'level': 'A',
  'name': '758ca7d590',
  'natScore': 66,
  'order': 1,
  'subject': 'English'},
 {'grade': 'AB3',
  'index': '56-4',
  'level': 'A',
  'name': '758ca7d590',
  'natScore': 63,
  'order': 1,
  'subject': 'Maths'},
 {'grade': 'AB2',
  'index': '56-6',
  'level': 'A',
  'name': '758ca7d590',
  'natScore': 37,
  'order': 1,
  'subject': 'Geography'},
 {'grade': 'AB2',
  'index': '56-10',
  'level': 'A',
  'name': '758ca7d590',
  'natScore': 58,
  'order': 1,
  'subject': 'Art'},
 {'grade': 'AC1',
  'index': '56-13',
  'level': 'A',
  'name': '758ca7d590',
  'natScore': 45,
  'order': 1,
  'subject': 'Chemistry'},
 {'grade': 'AB3',
  'index': '56-14',
  'level': 'A',
  'name': '758ca7d590',
  'natScore': 42,
  'order': 1,
  'subject': 'Ag. Science'},
 {'grade': 'AD1',
  'index': '57-3',
  'level': 'A',
  'name': '3bcdfc7945',
  'natScore': 11,
  'order': 1,
  'subject': 'English'},
 {'grade': 'GC2',
  'index': '57-4',
  'level': 'G',
  'name': '3bcdfc7945',
  'natScore': 58,
  'order': 1,
  'subject': 'Maths'},
 {'grade': 'AC3',
  'index': '57-6',
  'level': 'A',
  'name': '3bcdfc7945',
  'natScore': 47,
  'order': 1,
  'subject': 'Geography'},
 {'grade': 'AD1',
  'index': '57-10',
  'level': 'A',
  'name': '3bcdfc7945',
  'natScore': 94,
  'order': 1,
  'subject': 'Art'},
 {'grade': 'AD1',
  'index': '57-14',
  'level': 'A',
  'name': '3bcdfc7945',
  'natScore': 76,
  'order': 1,
  'subject': 'Ag. Science'},
 {'grade': 'AC3',
  'index': '57-15',
  'level': 'A',
  'name': '3bcdfc7945',
  'natScore': 7,
  'order': 1,
  'subject': 'Biology'},
 {'grade': 'GC3',
  'index': '58-3',
  'level': 'G',
  'name': 'c6ea25c4f3',
  'natScore': 70,
  'order': 1,
  'subject': 'English'},
 {'grade': 'GD1',
  'index': '58-5',
  'level': 'G',
  'name': 'c6ea25c4f3',
  'natScore': 83,
  'order': 1,
  'subject': 'History'},
 {'grade': 'GD3',
  'index': '58-6',
  'level': 'G',
  'name': 'c6ea25c4f3',
  'natScore': 52,
  'order': 1,
  'subject': 'Geography'},
 {'grade': 'GD3',
  'index': '58-10',
  'level': 'G',
  'name': 'c6ea25c4f3',
  'natScore': 60,
  'order': 1,
  'subject': 'Art'},
 {'grade': 'GC3',
  'index': '58-15',
  'level': 'G',
  'name': 'c6ea25c4f3',
  'natScore': 49,
  'order': 1,
  'subject': 'Biology'},
 {'grade': 'AC2',
  'index': '59-2',
  'level': 'A',
  'name': '5372b3209e',
  'natScore': 35,
  'order': 1,
  'subject': 'Irish'},
 {'grade': 'AC2',
  'index': '59-3',
  'level': 'A',
  'name': '5372b3209e',
  'natScore': 20,
  'order': 1,
  'subject': 'English'},
 {'grade': 'GC1',
  'index': '59-4',
  'level': 'G',
  'name': '5372b3209e',
  'natScore': 11,
  'order': 1,
  'subject': 'Maths'},
 {'grade': 'AB3',
  'index': '59-6',
  'level': 'A',
  'name': '5372b3209e',
  'natScore': 57,
  'order': 1,
  'subject': 'Geography'},
 {'grade': 'AD1',
  'index': '59-9',
  'level': 'A',
  'name': '5372b3209e',
  'natScore': 55,
  'order': 1,
  'subject': 'Spanish'},
 {'grade': 'AC2',
  'index': '59-14',
  'level': 'A',
  'name': '5372b3209e',
  'natScore': 77,
  'order': 1,
  'subject': 'Ag. Science'},
 {'grade': 'AB2',
  'index': '59-19',
  'level': 'A',
  'name': '5372b3209e',
  'natScore': 24,
  'order': 1,
  'subject': 'Business'},
 {'grade': 'GB3',
  'index': '60-2',
  'level': 'G',
  'name': 'a670d53746',
  'natScore': 22,
  'order': 1,
  'subject': 'Irish'},
 {'grade': 'AD2',
  'index': '60-3',
  'level': 'A',
  'name': 'a670d53746',
  'natScore': 72,
  'order': 1,
  'subject': 'English'},
 {'grade': 'GB1',
  'index': '60-4',
  'level': 'G',
  'name': 'a670d53746',
  'natScore': 98,
  'order': 1,
  'subject': 'Maths'},
 {'grade': 'AB3',
  'index': '60-6',
  'level': 'A',
  'name': 'a670d53746',
  'natScore': 16,
  'order': 1,
  'subject': 'Geography'},
 {'grade': 'AD2',
  'index': '60-7',
  'level': 'A',
  'name': 'a670d53746',
  'natScore': 59,
  'order': 1,
  'subject': 'French'},
 {'grade': 'AB2',
  'index': '60-15',
  'level': 'A',
  'name': 'a670d53746',
  'natScore': 45,
  'order': 1,
  'subject': 'Biology'},
 {'grade': 'AB3',
  'index': '60-18',
  'level': 'A',
  'name': 'a670d53746',
  'natScore': 8,
  'order': 1,
  'subject': 'Accounting'},
 {'grade': 'AC3',
  'index': '61-2',
  'level': 'A',
  'name': '3cbd1ef718',
  'natScore': 33,
  'order': 1,
  'subject': 'Irish'},
 {'grade': 'GA2',
  'index': '61-3',
  'level': 'G',
  'name': '3cbd1ef718',
  'natScore': 2,
  'order': 1,
  'subject': 'English'},
 {'grade': 'GB1',
  'index': '61-4',
  'level': 'G',
  'name': '3cbd1ef718',
  'natScore': 70,
  'order': 1,
  'subject': 'Maths'},
 {'grade': 'AB3',
  'index': '61-6',
  'level': 'A',
  'name': '3cbd1ef718',
  'natScore': 61,
  'order': 1,
  'subject': 'Geography'},
 {'grade': 'AB2',
  'index': '61-14',
  'level': 'A',
  'name': '3cbd1ef718',
  'natScore': 33,
  'order': 1,
  'subject': 'Ag. Science'},
 {'grade': 'AA2',
  'index': '61-15',
  'level': 'A',
  'name': '3cbd1ef718',
  'natScore': 61,
  'order': 1,
  'subject': 'Biology'},
 {'grade': 'AC1',
  'index': '61-16',
  'level': 'A',
  'name': '3cbd1ef718',
  'natScore': 13,
  'order': 1,
  'subject': 'Engineering'},
 {'grade': 'AC1',
  'index': '62-2',
  'level': 'A',
  'name': 'fcb7e3fe29',
  'natScore': 73,
  'order': 1,
  'subject': 'Irish'},
 {'grade': 'AB2',
  'index': '62-3',
  'level': 'A',
  'name': 'fcb7e3fe29',
  'natScore': 15,
  'order': 1,
  'subject': 'English'},
 {'grade': 'GA2',
  'index': '62-4',
  'level': 'G',
  'name': 'fcb7e3fe29',
  'natScore': 1,
  'order': 1,
  'subject': 'Maths'},
 {'grade': 'AB2',
  'index': '62-6',
  'level': 'A',
  'name': 'fcb7e3fe29',
  'natScore': 88,
  'order': 1,
  'subject': 'Geography'},
 {'grade': 'GB2',
  'index': '62-7',
  'level': 'G',
  'name': 'fcb7e3fe29',
  'natScore': 41,
  'order': 1,
  'subject': 'French'},
 {'grade': 'AB3',
  'index': '62-10',
  'level': 'A',
  'name': 'fcb7e3fe29',
  'natScore': 71,
  'order': 1,
  'subject': 'Art'},
 {'grade': 'AB2',
  'index': '62-14',
  'level': 'A',
  'name': 'fcb7e3fe29',
  'natScore': 83,
  'order': 1,
  'subject': 'Ag. Science'},
 {'grade': 'GC1',
  'index': '63-2',
  'level': 'G',
  'name': 'bb4e1ff824',
  'natScore': 55,
  'order': 1,
  'subject': 'Irish'},
 {'grade': 'GB3',
  'index': '63-3',
  'level': 'G',
  'name': 'bb4e1ff824',
  'natScore': 33,
  'order': 1,
  'subject': 'English'},
 {'grade': 'AD3',
  'index': '63-4',
  'level': 'A',
  'name': 'bb4e1ff824',
  'natScore': 18,
  'order': 1,
  'subject': 'Maths'},
 {'grade': 'AD3',
  'index': '63-6',
  'level': 'A',
  'name': 'bb4e1ff824',
  'natScore': 82,
  'order': 1,
  'subject': 'Geography'},
 {'grade': 'AC2',
  'index': '63-14',
  'level': 'A',
  'name': 'bb4e1ff824',
  'natScore': 10,
  'order': 1,
  'subject': 'Ag. Science'},
 {'grade': 'AD3',
  'index': '63-15',
  'level': 'A',
  'name': 'bb4e1ff824',
  'natScore': 93,
  'order': 1,
  'subject': 'Biology'},
 {'grade': 'AB3',
  'index': '63-16',
  'level': 'A',
  'name': 'bb4e1ff824',
  'natScore': 22,
  'order': 1,
  'subject': 'Engineering'},
 {'grade': 'GB3',
  'index': '64-2',
  'level': 'G',
  'name': 'd8d3edf1e3',
  'natScore': 52,
  'order': 1,
  'subject': 'Irish'},
 {'grade': 'AD1',
  'index': '64-3',
  'level': 'A',
  'name': 'd8d3edf1e3',
  'natScore': 57,
  'order': 1,
  'subject': 'English'},
 {'grade': 'GC2',
  'index': '64-4',
  'level': 'G',
  'name': 'd8d3edf1e3',
  'natScore': 61,
  'order': 1,
  'subject': 'Maths'},
 {'grade': 'AC3',
  'index': '64-6',
  'level': 'A',
  'name': 'd8d3edf1e3',
  'natScore': 61,
  'order': 1,
  'subject': 'Geography'},
 {'grade': 'GC2',
  'index': '64-7',
  'level': 'G',
  'name': 'd8d3edf1e3',
  'natScore': 89,
  'order': 1,
  'subject': 'French'},
 {'grade': 'AC2',
  'index': '64-15',
  'level': 'A',
  'name': 'd8d3edf1e3',
  'natScore': 76,
  'order': 1,
  'subject': 'Biology'},
 {'grade': 'AC1',
  'index': '64-16',
  'level': 'A',
  'name': 'd8d3edf1e3',
  'natScore': 91,
  'order': 1,
  'subject': 'Engineering'},
 {'grade': 'AD1',
  'index': '64-18',
  'level': 'A',
  'name': 'd8d3edf1e3',
  'natScore': 60,
  'order': 1,
  'subject': 'Accounting'},
 {'grade': 'GD1',
  'index': '65-2',
  'level': 'G',
  'name': 'b8263f4fdc',
  'natScore': 48,
  'order': 1,
  'subject': 'Irish'},
 {'grade': 'AD2',
  'index': '65-3',
  'level': 'A',
  'name': 'b8263f4fdc',
  'natScore': 8,
  'order': 1,
  'subject': 'English'},
 {'grade': 'GD1',
  'index': '65-4',
  'level': 'G',
  'name': 'b8263f4fdc',
  'natScore': 71,
  'order': 1,
  'subject': 'Maths'},
 {'grade': 'AD3',
  'index': '65-6',
  'level': 'A',
  'name': 'b8263f4fdc',
  'natScore': 35,
  'order': 1,
  'subject': 'Geography'},
 {'grade': 'AD3',
  'index': '65-14',
  'level': 'A',
  'name': 'b8263f4fdc',
  'natScore': 5,
  'order': 1,
  'subject': 'Ag. Science'},
 {'grade': 'AD3',
  'index': '65-16',
  'level': 'A',
  'name': 'b8263f4fdc',
  'natScore': 75,
  'order': 1,
  'subject': 'Engineering'},
 {'grade': 'AD3',
  'index': '65-22',
  'level': 'A',
  'name': 'b8263f4fdc',
  'natScore': 42,
  'order': 1,
  'subject': 'Home Ec.'},
 {'grade': 'AC2',
  'index': '66-2',
  'level': 'A',
  'name': '310b7a18af',
  'natScore': 22,
  'order': 1,
  'subject': 'Irish'},
 {'grade': 'AC2',
  'index': '66-3',
  'level': 'A',
  'name': '310b7a18af',
  'natScore': 3,
  'order': 1,
  'subject': 'English'},
 {'grade': 'AD1',
  'index': '66-4',
  'level': 'A',
  'name': '310b7a18af',
  'natScore': 30,
  'order': 1,
  'subject': 'Maths'},
 {'grade': 'AC3',
  'index': '66-5',
  'level': 'A',
  'name': '310b7a18af',
  'natScore': 94,
  'order': 1,
  'subject': 'History'},
 {'grade': 'GB3',
  'index': '66-7',
  'level': 'G',
  'name': '310b7a18af',
  'natScore': 35,
  'order': 1,
  'subject': 'French'},
 {'grade': 'AC2',
  'index': '66-14',
  'level': 'A',
  'name': '310b7a18af',
  'natScore': 25,
  'order': 1,
  'subject': 'Ag. Science'},
 {'grade': 'AC3',
  'index': '66-15',
  'level': 'A',
  'name': '310b7a18af',
  'natScore': 66,
  'order': 1,
  'subject': 'Biology'},
 {'grade': 'AB2',
  'index': '66-16',
  'level': 'A',
  'name': '310b7a18af',
  'natScore': 67,
  'order': 1,
  'subject': 'Engineering'},
 {'grade': 'AC1',
  'index': '67-2',
  'level': 'A',
  'name': '36381f79d1',
  'natScore': 7,
  'order': 1,
  'subject': 'Irish'},
 {'grade': 'AD2',
  'index': '67-3',
  'level': 'A',
  'name': '36381f79d1',
  'natScore': 0,
  'order': 1,
  'subject': 'English'},
 {'grade': 'GC3',
  'index': '67-4',
  'level': 'G',
  'name': '36381f79d1',
  'natScore': 31,
  'order': 1,
  'subject': 'Maths'},
 {'grade': 'AD2',
  'index': '67-6',
  'level': 'A',
  'name': '36381f79d1',
  'natScore': 26,
  'order': 1,
  'subject': 'Geography'},
 {'grade': 'GC3',
  'index': '67-7',
  'level': 'G',
  'name': '36381f79d1',
  'natScore': 57,
  'order': 1,
  'subject': 'French'},
 {'grade': 'AD3',
  'index': '67-15',
  'level': 'A',
  'name': '36381f79d1',
  'natScore': 38,
  'order': 1,
  'subject': 'Biology'},
 {'grade': 'AF',
  'index': '67-18',
  'level': 'A',
  'name': '36381f79d1',
  'natScore': 28,
  'order': 1,
  'subject': 'Accounting'},
 {'grade': 'GC1',
  'index': '68-2',
  'level': 'G',
  'name': 'fb8487a0c9',
  'natScore': 18,
  'order': 1,
  'subject': 'Irish'},
 {'grade': 'AD2',
  'index': '68-3',
  'level': 'A',
  'name': 'fb8487a0c9',
  'natScore': 18,
  'order': 1,
  'subject': 'English'},
 {'grade': 'AC3',
  'index': '68-4',
  'level': 'A',
  'name': 'fb8487a0c9',
  'natScore': 78,
  'order': 1,
  'subject': 'Maths'},
 {'grade': 'AC2',
  'index': '68-6',
  'level': 'A',
  'name': 'fb8487a0c9',
  'natScore': 51,
  'order': 1,
  'subject': 'Geography'},
 {'grade': 'AB2',
  'index': '68-15',
  'level': 'A',
  'name': 'fb8487a0c9',
  'natScore': 41,
  'order': 1,
  'subject': 'Biology'},
 {'grade': 'AB1',
  'index': '68-16',
  'level': 'A',
  'name': 'fb8487a0c9',
  'natScore': 71,
  'order': 1,
  'subject': 'Engineering'},
 {'grade': 'AB2',
  'index': '68-17',
  'level': 'A',
  'name': 'fb8487a0c9',
  'natScore': 17,
  'order': 1,
  'subject': 'Construction Studies'},
 {'grade': 'GC1',
  'index': '69-2',
  'level': 'G',
  'name': '047968a3da',
  'natScore': 40,
  'order': 1,
  'subject': 'Irish'},
 {'grade': 'AD1',
  'index': '69-3',
  'level': 'A',
  'name': '047968a3da',
  'natScore': 75,
  'order': 1,
  'subject': 'English'},
 {'grade': 'GC1',
  'index': '69-4',
  'level': 'G',
  'name': '047968a3da',
  'natScore': 4,
  'order': 1,
  'subject': 'Maths'},
 {'grade': 'AB3',
  'index': '69-6',
  'level': 'A',
  'name': '047968a3da',
  'natScore': 22,
  'order': 1,
  'subject': 'Geography'},
 {'grade': 'AC3',
  'index': '69-7',
  'level': 'A',
  'name': '047968a3da',
  'natScore': 92,
  'order': 1,
  'subject': 'French'},
 {'grade': 'AC2',
  'index': '69-10',
  'level': 'A',
  'name': '047968a3da',
  'natScore': 1,
  'order': 1,
  'subject': 'Art'},
 {'grade': 'AC1',
  'index': '69-15',
  'level': 'A',
  'name': '047968a3da',
  'natScore': 54,
  'order': 1,
  'subject': 'Biology'},
 {'grade': 'AB2',
  'index': '70-2',
  'level': 'A',
  'name': 'a72cdca062',
  'natScore': 35,
  'order': 1,
  'subject': 'Irish'},
 {'grade': 'AB3',
  'index': '70-3',
  'level': 'A',
  'name': 'a72cdca062',
  'natScore': 61,
  'order': 1,
  'subject': 'English'},
 {'grade': 'GB3',
  'index': '70-4',
  'level': 'G',
  'name': 'a72cdca062',
  'natScore': 15,
  'order': 1,
  'subject': 'Maths'},
 {'grade': 'AC2',
  'index': '70-6',
  'level': 'A',
  'name': 'a72cdca062',
  'natScore': 59,
  'order': 1,
  'subject': 'Geography'},
 {'grade': 'AB2',
  'index': '70-15',
  'level': 'A',
  'name': 'a72cdca062',
  'natScore': 2,
  'order': 1,
  'subject': 'Biology'},
 {'grade': 'AB2',
  'index': '70-18',
  'level': 'A',
  'name': 'a72cdca062',
  'natScore': 80,
  'order': 1,
  'subject': 'Accounting'},
 {'grade': 'AB3',
  'index': '70-19',
  'level': 'A',
  'name': 'a72cdca062',
  'natScore': 89,
  'order': 1,
  'subject': 'Business'},
 {'grade': 'GD1',
  'index': '71-2',
  'level': 'G',
  'name': '6069b69a0b',
  'natScore': 78,
  'order': 1,
  'subject': 'Irish'},
 {'grade': 'AD2',
  'index': '71-3',
  'level': 'A',
  'name': '6069b69a0b',
  'natScore': 67,
  'order': 1,
  'subject': 'English'},
 {'grade': 'GC1',
  'index': '71-4',
  'level': 'G',
  'name': '6069b69a0b',
  'natScore': 44,
  'order': 1,
  'subject': 'Maths'},
 {'grade': 'AB1',
  'index': '71-10',
  'level': 'A',
  'name': '6069b69a0b',
  'natScore': 67,
  'order': 1,
  'subject': 'Art'},
 {'grade': 'AD1',
  'index': '71-14',
  'level': 'A',
  'name': '6069b69a0b',
  'natScore': 40,
  'order': 1,
  'subject': 'Ag. Science'},
 {'grade': 'AD3',
  'index': '71-15',
  'level': 'A',
  'name': '6069b69a0b',
  'natScore': 97,
  'order': 1,
  'subject': 'Biology'},
 {'grade': 'AD1',
  'index': '71-22',
  'level': 'A',
  'name': '6069b69a0b',
  'natScore': 69,
  'order': 1,
  'subject': 'Home Ec.'},
 {'grade': 'AC3',
  'index': '72-2',
  'level': 'A',
  'name': 'a644565d3b',
  'natScore': 49,
  'order': 1,
  'subject': 'Irish'},
 {'grade': 'AC2',
  'index': '72-3',
  'level': 'A',
  'name': 'a644565d3b',
  'natScore': 27,
  'order': 1,
  'subject': 'English'},
 {'grade': 'GC2',
  'index': '72-4',
  'level': 'G',
  'name': 'a644565d3b',
  'natScore': 60,
  'order': 1,
  'subject': 'Maths'},
 {'grade': 'AC3',
  'index': '72-6',
  'level': 'A',
  'name': 'a644565d3b',
  'natScore': 68,
  'order': 1,
  'subject': 'Geography'},
 {'grade': 'AC2',
  'index': '72-7',
  'level': 'A',
  'name': 'a644565d3b',
  'natScore': 36,
  'order': 1,
  'subject': 'French'},
 {'grade': 'AC3',
  'index': '72-14',
  'level': 'A',
  'name': 'a644565d3b',
  'natScore': 5,
  'order': 1,
  'subject': 'Ag. Science'},
 {'grade': 'AD1',
  'index': '72-21',
  'level': 'A',
  'name': 'a644565d3b',
  'natScore': 23,
  'order': 1,
  'subject': 'Music'},
 {'grade': 'AC1',
  'index': '73-2',
  'level': 'A',
  'name': 'f0852083e5',
  'natScore': 84,
  'order': 1,
  'subject': 'Irish'},
 {'grade': 'AB2',
  'index': '73-3',
  'level': 'A',
  'name': 'f0852083e5',
  'natScore': 17,
  'order': 1,
  'subject': 'English'},
 {'grade': 'AC2',
  'index': '73-4',
  'level': 'A',
  'name': 'f0852083e5',
  'natScore': 71,
  'order': 1,
  'subject': 'Maths'},
 {'grade': 'AB3',
  'index': '73-6',
  'level': 'A',
  'name': 'f0852083e5',
  'natScore': 46,
  'order': 1,
  'subject': 'Geography'},
 {'grade': 'GC1',
  'index': '73-7',
  'level': 'G',
  'name': 'f0852083e5',
  'natScore': 27,
  'order': 1,
  'subject': 'French'},
 {'grade': 'AC2',
  'index': '73-18',
  'level': 'A',
  'name': 'f0852083e5',
  'natScore': 80,
  'order': 1,
  'subject': 'Accounting'},
 {'grade': 'AC3',
  'index': '73-20',
  'level': 'A',
  'name': 'f0852083e5',
  'natScore': 0,
  'order': 1,
  'subject': 'Economics'},
 {'grade': 'GC2',
  'index': '74-2',
  'level': 'G',
  'name': 'a4ed825b7b',
  'natScore': 74,
  'order': 1,
  'subject': 'Irish'},
 {'grade': 'AD1',
  'index': '74-3',
  'level': 'A',
  'name': 'a4ed825b7b',
  'natScore': 78,
  'order': 1,
  'subject': 'English'},
 {'grade': 'GB1',
  'index': '74-4',
  'level': 'G',
  'name': 'a4ed825b7b',
  'natScore': 40,
  'order': 1,
  'subject': 'Maths'},
 {'grade': 'AC2',
  'index': '74-5',
  'level': 'A',
  'name': 'a4ed825b7b',
  'natScore': 36,
  'order': 1,
  'subject': 'History'},
 {'grade': 'AC1',
  'index': '74-6',
  'level': 'A',
  'name': 'a4ed825b7b',
  'natScore': 67,
  'order': 1,
  'subject': 'Geography'},
 {'grade': 'GB3',
  'index': '74-7',
  'level': 'G',
  'name': 'a4ed825b7b',
  'natScore': 98,
  'order': 1,
  'subject': 'French'},
 {'grade': 'AD1',
  'index': '74-15',
  'level': 'A',
  'name': 'a4ed825b7b',
  'natScore': 59,
  'order': 1,
  'subject': 'Biology'},
 {'grade': 'AC1',
  'index': '74-19',
  'level': 'A',
  'name': 'a4ed825b7b',
  'natScore': 85,
  'order': 1,
  'subject': 'Business'},
 {'grade': 'AB1',
  'index': '75-2',
  'level': 'A',
  'name': '772b184068',
  'natScore': 24,
  'order': 1,
  'subject': 'Irish'},
 {'grade': 'AC2',
  'index': '75-3',
  'level': 'A',
  'name': '772b184068',
  'natScore': 40,
  'order': 1,
  'subject': 'English'},
 {'grade': 'AC2',
  'index': '75-4',
  'level': 'A',
  'name': '772b184068',
  'natScore': 45,
  'order': 1,
  'subject': 'Maths'},
 {'grade': 'AB2',
  'index': '75-8',
  'level': 'A',
  'name': '772b184068',
  'natScore': 15,
  'order': 1,
  'subject': 'German'},
 {'grade': 'AB3',
  'index': '75-13',
  'level': 'A',
  'name': '772b184068',
  'natScore': 24,
  'order': 1,
  'subject': 'Chemistry'},
 {'grade': 'AA2',
  'index': '75-15',
  'level': 'A',
  'name': '772b184068',
  'natScore': 28,
  'order': 1,
  'subject': 'Biology'},
 {'grade': 'AA2',
  'index': '75-17',
  'level': 'A',
  'name': '772b184068',
  'natScore': 9,
  'order': 1,
  'subject': 'Construction Studies'},
 {'grade': 'AA1',
  'index': '76-2',
  'level': 'A',
  'name': 'd839b418c7',
  'natScore': 54,
  'order': 1,
  'subject': 'Irish'},
 {'grade': 'AA1',
  'index': '76-3',
  'level': 'A',
  'name': 'd839b418c7',
  'natScore': 18,
  'order': 1,
  'subject': 'English'},
 {'grade': 'AA1',
  'index': '76-4',
  'level': 'A',
  'name': 'd839b418c7',
  'natScore': 24,
  'order': 1,
  'subject': 'Maths'},
 {'grade': 'AA1',
  'index': '76-9',
  'level': 'A',
  'name': 'd839b418c7',
  'natScore': 47,
  'order': 1,
  'subject': 'Spanish'},
 {'grade': 'AA2',
  'index': '76-11',
  'level': 'A',
  'name': 'd839b418c7',
  'natScore': 59,
  'order': 1,
  'subject': 'Applied Maths'},
 {'grade': 'AA1',
  'index': '76-12',
  'level': 'A',
  'name': 'd839b418c7',
  'natScore': 44,
  'order': 1,
  'subject': 'Physics'},
 {'grade': 'AA2',
  'index': '76-13',
  'level': 'A',
  'name': 'd839b418c7',
  'natScore': 56,
  'order': 1,
  'subject': 'Chemistry'},
 {'grade': 'AA2',
  'index': '76-18',
  'level': 'A',
  'name': 'd839b418c7',
  'natScore': 55,
  'order': 1,
  'subject': 'Accounting'},
 {'grade': 'GB3',
  'index': '77-3',
  'level': 'G',
  'name': '0f2e2ac409',
  'natScore': 7,
  'order': 1,
  'subject': 'English'},
 {'grade': 'GD1',
  'index': '77-4',
  'level': 'G',
  'name': '0f2e2ac409',
  'natScore': 31,
  'order': 1,
  'subject': 'Maths'},
 {'grade': 'GB2',
  'index': '77-5',
  'level': 'G',
  'name': '0f2e2ac409',
  'natScore': 38,
  'order': 1,
  'subject': 'History'},
 {'grade': 'AD2',
  'index': '77-6',
  'level': 'A',
  'name': '0f2e2ac409',
  'natScore': 22,
  'order': 1,
  'subject': 'Geography'},
 {'grade': 'AD3',
  'index': '77-10',
  'level': 'A',
  'name': '0f2e2ac409',
  'natScore': 82,
  'order': 1,
  'subject': 'Art'},
 {'grade': 'AD3',
  'index': '77-15',
  'level': 'A',
  'name': '0f2e2ac409',
  'natScore': 79,
  'order': 1,
  'subject': 'Biology'},
 {'grade': 'GC3',
  'index': '78-2',
  'level': 'G',
  'name': 'fe77bb963e',
  'natScore': 86,
  'order': 1,
  'subject': 'Irish'},
 {'grade': 'AB2',
  'index': '78-3',
  'level': 'A',
  'name': 'fe77bb963e',
  'natScore': 47,
  'order': 1,
  'subject': 'English'},
 {'grade': 'GB1',
  'index': '78-4',
  'level': 'G',
  'name': 'fe77bb963e',
  'natScore': 93,
  'order': 1,
  'subject': 'Maths'},
 {'grade': 'AC1',
  'index': '78-5',
  'level': 'A',
  'name': 'fe77bb963e',
  'natScore': 42,
  'order': 1,
  'subject': 'History'},
 {'grade': 'AC2',
  'index': '78-10',
  'level': 'A',
  'name': 'fe77bb963e',
  'natScore': 34,
  'order': 1,
  'subject': 'Art'},
 {'grade': 'AC2',
  'index': '78-15',
  'level': 'A',
  'name': 'fe77bb963e',
  'natScore': 8,
  'order': 1,
  'subject': 'Biology'},
 {'grade': 'AB2',
  'index': '78-21',
  'level': 'A',
  'name': 'fe77bb963e',
  'natScore': 29,
  'order': 1,
  'subject': 'Music'},
 {'grade': 'GC3',
  'index': '79-2',
  'level': 'G',
  'name': '43177e96ba',
  'natScore': 10,
  'order': 1,
  'subject': 'Irish'},
 {'grade': 'AC2',
  'index': '79-3',
  'level': 'A',
  'name': '43177e96ba',
  'natScore': 46,
  'order': 1,
  'subject': 'English'},
 {'grade': 'GC2',
  'index': '79-4',
  'level': 'G',
  'name': '43177e96ba',
  'natScore': 63,
  'order': 1,
  'subject': 'Maths'},
 {'grade': 'AD1',
  'index': '79-6',
  'level': 'A',
  'name': '43177e96ba',
  'natScore': 81,
  'order': 1,
  'subject': 'Geography'},
 {'grade': 'AC2',
  'index': '79-10',
  'level': 'A',
  'name': '43177e96ba',
  'natScore': 83,
  'order': 1,
  'subject': 'Art'},
 {'grade': 'AE',
  'index': '79-15',
  'level': 'A',
  'name': '43177e96ba',
  'natScore': 98,
  'order': 1,
  'subject': 'Biology'},
 {'grade': 'AD2',
  'index': '79-20',
  'level': 'A',
  'name': '43177e96ba',
  'natScore': 93,
  'order': 1,
  'subject': 'Economics'},
 {'grade': 'AD1',
  'index': '80-2',
  'level': 'A',
  'name': 'b7ca65b20d',
  'natScore': 2,
  'order': 1,
  'subject': 'Irish'},
 {'grade': 'AD1',
  'index': '80-3',
  'level': 'A',
  'name': 'b7ca65b20d',
  'natScore': 88,
  'order': 1,
  'subject': 'English'},
 {'grade': 'GC3',
  'index': '80-4',
  'level': 'G',
  'name': 'b7ca65b20d',
  'natScore': 77,
  'order': 1,
  'subject': 'Maths'},
 {'grade': 'AD3',
  'index': '80-5',
  'level': 'A',
  'name': 'b7ca65b20d',
  'natScore': 83,
  'order': 1,
  'subject': 'History'},
 {'grade': 'AE',
  'index': '80-9',
  'level': 'A',
  'name': 'b7ca65b20d',
  'natScore': 21,
  'order': 1,
  'subject': 'Spanish'},
 {'grade': 'AD1',
  'index': '80-15',
  'level': 'A',
  'name': 'b7ca65b20d',
  'natScore': 23,
  'order': 1,
  'subject': 'Biology'},
 {'grade': 'AE',
  'index': '80-20',
  'level': 'A',
  'name': 'b7ca65b20d',
  'natScore': 44,
  'order': 1,
  'subject': 'Economics'},
 {'grade': 'GA2',
  'index': '81-2',
  'level': 'G',
  'name': 'ce2948216b',
  'natScore': 90,
  'order': 1,
  'subject': 'Irish'},
 {'grade': 'AC2',
  'index': '81-3',
  'level': 'A',
  'name': 'ce2948216b',
  'natScore': 20,
  'order': 1,
  'subject': 'English'},
 {'grade': 'GB3',
  'index': '81-4',
  'level': 'G',
  'name': 'ce2948216b',
  'natScore': 87,
  'order': 1,
  'subject': 'Maths'},
 {'grade': 'AB3',
  'index': '81-6',
  'level': 'A',
  'name': 'ce2948216b',
  'natScore': 32,
  'order': 1,
  'subject': 'Geography'},
 {'grade': 'AC2',
  'index': '81-9',
  'level': 'A',
  'name': 'ce2948216b',
  'natScore': 26,
  'order': 1,
  'subject': 'Spanish'},
 {'grade': 'AB2',
  'index': '81-15',
  'level': 'A',
  'name': 'ce2948216b',
  'natScore': 79,
  'order': 1,
  'subject': 'Biology'},
 {'grade': 'AB3',
  'index': '81-22',
  'level': 'A',
  'name': 'ce2948216b',
  'natScore': 8,
  'order': 1,
  'subject': 'Home Ec.'},
 {'grade': 'GC2',
  'index': '82-3',
  'level': 'G',
  'name': 'f767188eac',
  'natScore': 9,
  'order': 1,
  'subject': 'English'},
 {'grade': 'AE',
  'index': '82-6',
  'level': 'A',
  'name': 'f767188eac',
  'natScore': 49,
  'order': 1,
  'subject': 'Geography'},
 {'grade': 'GC2',
  'index': '82-9',
  'level': 'G',
  'name': 'f767188eac',
  'natScore': 54,
  'order': 1,
  'subject': 'Spanish'},
 {'grade': 'GD3',
  'index': '82-15',
  'level': 'G',
  'name': 'f767188eac',
  'natScore': 96,
  'order': 1,
  'subject': 'Biology'},
 {'grade': 'AB3',
  'index': '82-23',
  'level': 'A',
  'name': 'f767188eac',
  'natScore': 73,
  'order': 1,
  'subject': 'Polish'},
 {'grade': 'GC1',
  'index': '83-3',
  'level': 'G',
  'name': '130c90416d',
  'natScore': 7,
  'order': 1,
  'subject': 'English'},
 {'grade': 'GC3',
  'index': '83-5',
  'level': 'G',
  'name': '130c90416d',
  'natScore': 13,
  'order': 1,
  'subject': 'History'},
 {'grade': 'GC2',
  'index': '83-8',
  'level': 'G',
  'name': '130c90416d',
  'natScore': 83,
  'order': 1,
  'subject': 'German'},
 {'grade': 'GC3',
  'index': '83-14',
  'level': 'G',
  'name': '130c90416d',
  'natScore': 46,
  'order': 1,
  'subject': 'Ag. Science'},
 {'grade': 'GF',
  'index': '83-15',
  'level': 'G',
  'name': '130c90416d',
  'natScore': 89,
  'order': 1,
  'subject': 'Biology'},
 {'grade': 'GD1',
  'index': '84-2',
  'level': 'G',
  'name': 'ae2b31ae59',
  'natScore': 54,
  'order': 1,
  'subject': 'Irish'},
 {'grade': 'GB3',
  'index': '84-3',
  'level': 'G',
  'name': 'ae2b31ae59',
  'natScore': 50,
  'order': 1,
  'subject': 'English'},
 {'grade': 'GC3',
  'index': '84-4',
  'level': 'G',
  'name': 'ae2b31ae59',
  'natScore': 100,
  'order': 1,
  'subject': 'Maths'},
 {'grade': 'AC3',
  'index': '84-6',
  'level': 'A',
  'name': 'ae2b31ae59',
  'natScore': 97,
  'order': 1,
  'subject': 'Geography'},
 {'grade': 'GC3',
  'index': '84-14',
  'level': 'G',
  'name': 'ae2b31ae59',
  'natScore': 47,
  'order': 1,
  'subject': 'Ag. Science'},
 {'grade': 'AD1',
  'index': '84-16',
  'level': 'A',
  'name': 'ae2b31ae59',
  'natScore': 33,
  'order': 1,
  'subject': 'Engineering'},
 {'grade': 'AD2',
  'index': '84-22',
  'level': 'A',
  'name': 'ae2b31ae59',
  'natScore': 56,
  'order': 1,
  'subject': 'Home Ec.'},
 {'grade': 'AC3',
  'index': '85-2',
  'level': 'A',
  'name': 'ae4e646bde',
  'natScore': 57,
  'order': 1,
  'subject': 'Irish'},
 {'grade': 'AC3',
  'index': '85-3',
  'level': 'A',
  'name': 'ae4e646bde',
  'natScore': 57,
  'order': 1,
  'subject': 'English'},
 {'grade': 'GB3',
  'index': '85-4',
  'level': 'G',
  'name': 'ae4e646bde',
  'natScore': 36,
  'order': 1,
  'subject': 'Maths'},
 {'grade': 'AD2',
  'index': '85-9',
  'level': 'A',
  'name': 'ae4e646bde',
  'natScore': 86,
  'order': 1,
  'subject': 'Spanish'},
 {'grade': 'AC1',
  'index': '85-14',
  'level': 'A',
  'name': 'ae4e646bde',
  'natScore': 18,
  'order': 1,
  'subject': 'Ag. Science'},
 {'grade': 'AD1',
  'index': '85-15',
  'level': 'A',
  'name': 'ae4e646bde',
  'natScore': 72,
  'order': 1,
  'subject': 'Biology'},
 {'grade': 'AD3',
  'index': '85-18',
  'level': 'A',
  'name': 'ae4e646bde',
  'natScore': 99,
  'order': 1,
  'subject': 'Accounting'},
 {'grade': 'AE',
  'index': '86-3',
  'level': 'A',
  'name': '9221f5c30d',
  'natScore': 14,
  'order': 1,
  'subject': 'English'},
 {'grade': 'GC2',
  'index': '86-4',
  'level': 'G',
  'name': '9221f5c30d',
  'natScore': 38,
  'order': 1,
  'subject': 'Maths'},
 {'grade': 'GA2',
  'index': '86-5',
  'level': 'G',
  'name': '9221f5c30d',
  'natScore': 59,
  'order': 1,
  'subject': 'History'},
 {'grade': 'GB3',
  'index': '86-6',
  'level': 'G',
  'name': '9221f5c30d',
  'natScore': 12,
  'order': 1,
  'subject': 'Geography'},
 {'grade': 'GB1',
  'index': '86-12',
  'level': 'G',
  'name': '9221f5c30d',
  'natScore': 74,
  'order': 1,
  'subject': 'Physics'},
 {'grade': 'GB3',
  'index': '86-13',
  'level': 'G',
  'name': '9221f5c30d',
  'natScore': 28,
  'order': 1,
  'subject': 'Chemistry'},
 {'grade': 'GD2',
  'index': '87-2',
  'level': 'G',
  'name': '66bde841c0',
  'natScore': 55,
  'order': 1,
  'subject': 'Irish'},
 {'grade': 'GA2',
  'index': '87-3',
  'level': 'G',
  'name': '66bde841c0',
  'natScore': 63,
  'order': 1,
  'subject': 'English'},
 {'grade': 'GC1',
  'index': '87-4',
  'level': 'G',
  'name': '66bde841c0',
  'natScore': 61,
  'order': 1,
  'subject': 'Maths'},
 {'grade': 'AD1',
  'index': '87-6',
  'level': 'A',
  'name': '66bde841c0',
  'natScore': 1,
  'order': 1,
  'subject': 'Geography'},
 {'grade': 'GC2',
  'index': '87-14',
  'level': 'G',
  'name': '66bde841c0',
  'natScore': 20,
  'order': 1,
  'subject': 'Ag. Science'},
 {'grade': 'AD1',
  'index': '87-15',
  'level': 'A',
  'name': '66bde841c0',
  'natScore': 55,
  'order': 1,
  'subject': 'Biology'},
 {'grade': 'AB3',
  'index': '87-16',
  'level': 'A',
  'name': '66bde841c0',
  'natScore': 27,
  'order': 1,
  'subject': 'Engineering'},
 {'grade': 'AC2',
  'index': '88-2',
  'level': 'A',
  'name': '8d0aded97b',
  'natScore': 27,
  'order': 1,
  'subject': 'Irish'},
 {'grade': 'AC3',
  'index': '88-3',
  'level': 'A',
  'name': '8d0aded97b',
  'natScore': 86,
  'order': 1,
  'subject': 'English'},
 {'grade': 'GB3',
  'index': '88-4',
  'level': 'G',
  'name': '8d0aded97b',
  'natScore': 38,
  'order': 1,
  'subject': 'Maths'},
 {'grade': 'AC1',
  'index': '88-6',
  'level': 'A',
  'name': '8d0aded97b',
  'natScore': 74,
  'order': 1,
  'subject': 'Geography'},
 {'grade': 'AC3',
  'index': '88-9',
  'level': 'A',
  'name': '8d0aded97b',
  'natScore': 97,
  'order': 1,
  'subject': 'Spanish'},
 {'grade': 'AC1',
  'index': '88-15',
  'level': 'A',
  'name': '8d0aded97b',
  'natScore': 77,
  'order': 1,
  'subject': 'Biology'},
 {'grade': 'AB2',
  'index': '88-22',
  'level': 'A',
  'name': '8d0aded97b',
  'natScore': 35,
  'order': 1,
  'subject': 'Home Ec.'},
 {'grade': 'AA2',
  'index': '89-2',
  'level': 'A',
  'name': 'd5c037423c',
  'natScore': 70,
  'order': 1,
  'subject': 'Irish'},
 {'grade': 'AA2',
  'index': '89-3',
  'level': 'A',
  'name': 'd5c037423c',
  'natScore': 30,
  'order': 1,
  'subject': 'English'},
 {'grade': 'AC1',
  'index': '89-4',
  'level': 'A',
  'name': 'd5c037423c',
  'natScore': 79,
  'order': 1,
  'subject': 'Maths'},
 {'grade': 'AA2',
  'index': '89-8',
  'level': 'A',
  'name': 'd5c037423c',
  'natScore': 61,
  'order': 1,
  'subject': 'German'},
 {'grade': 'AA2',
  'index': '89-13',
  'level': 'A',
  'name': 'd5c037423c',
  'natScore': 15,
  'order': 1,
  'subject': 'Chemistry'},
 {'grade': 'AA1',
  'index': '89-14',
  'level': 'A',
  'name': 'd5c037423c',
  'natScore': 29,
  'order': 1,
  'subject': 'Ag. Science'},
 {'grade': 'AA1',
  'index': '89-15',
  'level': 'A',
  'name': 'd5c037423c',
  'natScore': 88,
  'order': 1,
  'subject': 'Biology'},
 {'grade': 'AD1',
  'index': '90-2',
  'level': 'A',
  'name': '41c6dc8c8d',
  'natScore': 77,
  'order': 1,
  'subject': 'Irish'},
 {'grade': 'AB3',
  'index': '90-3',
  'level': 'A',
  'name': '41c6dc8c8d',
  'natScore': 92,
  'order': 1,
  'subject': 'English'},
 {'grade': 'GB2',
  'index': '90-4',
  'level': 'G',
  'name': '41c6dc8c8d',
  'natScore': 58,
  'order': 1,
  'subject': 'Maths'},
 {'grade': 'AB3',
  'index': '90-5',
  'level': 'A',
  'name': '41c6dc8c8d',
  'natScore': 47,
  'order': 1,
  'subject': 'History'},
 {'grade': 'AC2',
  'index': '90-6',
  'level': 'A',
  'name': '41c6dc8c8d',
  'natScore': 73,
  'order': 1,
  'subject': 'Geography'},
 {'grade': 'AC3',
  'index': '90-14',
  'level': 'A',
  'name': '41c6dc8c8d',
  'natScore': 62,
  'order': 1,
  'subject': 'Ag. Science'},
 {'grade': 'AC2',
  'index': '90-15',
  'level': 'A',
  'name': '41c6dc8c8d',
  'natScore': 6,
  'order': 1,
  'subject': 'Biology'},
 {'grade': 'GC1',
  'index': '91-2',
  'level': 'G',
  'name': '7bf42ff25e',
  'natScore': 45,
  'order': 1,
  'subject': 'Irish'},
 {'grade': 'AC3',
  'index': '91-3',
  'level': 'A',
  'name': '7bf42ff25e',
  'natScore': 33,
  'order': 1,
  'subject': 'English'},
 {'grade': 'GB3',
  'index': '91-4',
  'level': 'G',
  'name': '7bf42ff25e',
  'natScore': 37,
  'order': 1,
  'subject': 'Maths'},
 {'grade': 'AC3',
  'index': '91-10',
  'level': 'A',
  'name': '7bf42ff25e',
  'natScore': 25,
  'order': 1,
  'subject': 'Art'},
 {'grade': 'AC2',
  'index': '91-15',
  'level': 'A',
  'name': '7bf42ff25e',
  'natScore': 25,
  'order': 1,
  'subject': 'Biology'},
 {'grade': 'AB2',
  'index': '91-16',
  'level': 'A',
  'name': '7bf42ff25e',
  'natScore': 84,
  'order': 1,
  'subject': 'Engineering'},
 {'grade': 'AB1',
  'index': '91-17',
  'level': 'A',
  'name': '7bf42ff25e',
  'natScore': 64,
  'order': 1,
  'subject': 'Construction Studies'},
 {'grade': 'AC3',
  'index': '92-2',
  'level': 'A',
  'name': 'bf07e47166',
  'natScore': 11,
  'order': 1,
  'subject': 'Irish'},
 {'grade': 'AD2',
  'index': '92-3',
  'level': 'A',
  'name': 'bf07e47166',
  'natScore': 73,
  'order': 1,
  'subject': 'English'},
 {'grade': 'AD1',
  'index': '92-4',
  'level': 'A',
  'name': 'bf07e47166',
  'natScore': 95,
  'order': 1,
  'subject': 'Maths'},
 {'grade': 'GC1',
  'index': '92-9',
  'level': 'G',
  'name': 'bf07e47166',
  'natScore': 67,
  'order': 1,
  'subject': 'Spanish'},
 {'grade': 'AE',
  'index': '92-15',
  'level': 'A',
  'name': 'bf07e47166',
  'natScore': 54,
  'order': 1,
  'subject': 'Biology'},
 {'grade': 'AC1',
  'index': '92-17',
  'level': 'A',
  'name': 'bf07e47166',
  'natScore': 43,
  'order': 1,
  'subject': 'Construction Studies'},
 {'grade': 'AC3',
  'index': '92-19',
  'level': 'A',
  'name': 'bf07e47166',
  'natScore': 51,
  'order': 1,
  'subject': 'Business'},
 {'grade': 'GA1',
  'index': '93-2',
  'level': 'G',
  'name': '45a47f4f1d',
  'natScore': 33,
  'order': 1,
  'subject': 'Irish'},
 {'grade': 'AB1',
  'index': '93-3',
  'level': 'A',
  'name': '45a47f4f1d',
  'natScore': 46,
  'order': 1,
  'subject': 'English'},
 {'grade': 'AB3',
  'index': '93-4',
  'level': 'A',
  'name': '45a47f4f1d',
  'natScore': 24,
  'order': 1,
  'subject': 'Maths'},
 {'grade': 'AB1',
  'index': '93-7',
  'level': 'A',
  'name': '45a47f4f1d',
  'natScore': 38,
  'order': 1,
  'subject': 'French'},
 {'grade': 'AB1',
  'index': '93-13',
  'level': 'A',
  'name': '45a47f4f1d',
  'natScore': 53,
  'order': 1,
  'subject': 'Chemistry'},
 {'grade': 'AA2',
  'index': '93-14',
  'level': 'A',
  'name': '45a47f4f1d',
  'natScore': 42,
  'order': 1,
  'subject': 'Ag. Science'},
 {'grade': 'AA2',
  'index': '93-15',
  'level': 'A',
  'name': '45a47f4f1d',
  'natScore': 77,
  'order': 1,
  'subject': 'Biology'},
 {'grade': 'AA2',
  'index': '93-20',
  'level': 'A',
  'name': '45a47f4f1d',
  'natScore': 33,
  'order': 1,
  'subject': 'Economics'},
 {'grade': 'AC1',
  'index': '94-2',
  'level': 'A',
  'name': '0af29778dd',
  'natScore': 99,
  'order': 1,
  'subject': 'Irish'},
 {'grade': 'AD3',
  'index': '94-3',
  'level': 'A',
  'name': '0af29778dd',
  'natScore': 97,
  'order': 1,
  'subject': 'English'},
 {'grade': 'GB1',
  'index': '94-4',
  'level': 'G',
  'name': '0af29778dd',
  'natScore': 16,
  'order': 1,
  'subject': 'Maths'},
 {'grade': 'AC1',
  'index': '94-6',
  'level': 'A',
  'name': '0af29778dd',
  'natScore': 82,
  'order': 1,
  'subject': 'Geography'},
 {'grade': 'GB2',
  'index': '94-9',
  'level': 'G',
  'name': '0af29778dd',
  'natScore': 80,
  'order': 1,
  'subject': 'Spanish'},
 {'grade': 'AA2',
  'index': '94-16',
  'level': 'A',
  'name': '0af29778dd',
  'natScore': 64,
  'order': 1,
  'subject': 'Engineering'},
 {'grade': 'AA1',
  'index': '94-17',
  'level': 'A',
  'name': '0af29778dd',
  'natScore': 55,
  'order': 1,
  'subject': 'Construction Studies'},
 {'grade': 'GB2',
  'index': '95-2',
  'level': 'G',
  'name': '0756102183',
  'natScore': 97,
  'order': 1,
  'subject': 'Irish'},
 {'grade': 'GB2',
  'index': '95-3',
  'level': 'G',
  'name': '0756102183',
  'natScore': 2,
  'order': 1,
  'subject': 'English'},
 {'grade': 'AD1',
  'index': '95-4',
  'level': 'A',
  'name': '0756102183',
  'natScore': 98,
  'order': 1,
  'subject': 'Maths'},
 {'grade': 'AC2',
  'index': '95-6',
  'level': 'A',
  'name': '0756102183',
  'natScore': 60,
  'order': 1,
  'subject': 'Geography'},
 {'grade': 'AC1',
  'index': '95-8',
  'level': 'A',
  'name': '0756102183',
  'natScore': 88,
  'order': 1,
  'subject': 'German'},
 {'grade': 'AB1',
  'index': '95-14',
  'level': 'A',
  'name': '0756102183',
  'natScore': 88,
  'order': 1,
  'subject': 'Ag. Science'},
 {'grade': 'AC1',
  'index': '95-15',
  'level': 'A',
  'name': '0756102183',
  'natScore': 74,
  'order': 1,
  'subject': 'Biology'},
 {'grade': 'GB2',
  'index': '96-2',
  'level': 'G',
  'name': '1e48810bb4',
  'natScore': 16,
  'order': 1,
  'subject': 'Irish'},
 {'grade': 'AB1',
  'index': '96-3',
  'level': 'A',
  'name': '1e48810bb4',
  'natScore': 12,
  'order': 1,
  'subject': 'English'},
 {'grade': 'AA2',
  'index': '96-4',
  'level': 'A',
  'name': '1e48810bb4',
  'natScore': 25,
  'order': 1,
  'subject': 'Maths'},
 {'grade': 'AC1',
  'index': '96-7',
  'level': 'A',
  'name': '1e48810bb4',
  'natScore': 92,
  'order': 1,
  'subject': 'French'},
 {'grade': 'AB1',
  'index': '96-12',
  'level': 'A',
  'name': '1e48810bb4',
  'natScore': 93,
  'order': 1,
  'subject': 'Physics'},
 {'grade': 'AB3',
  'index': '96-14',
  'level': 'A',
  'name': '1e48810bb4',
  'natScore': 67,
  'order': 1,
  'subject': 'Ag. Science'},
 {'grade': 'AB2',
  'index': '96-18',
  'level': 'A',
  'name': '1e48810bb4',
  'natScore': 5,
  'order': 1,
  'subject': 'Accounting'},
 {'grade': 'AA2',
  'index': '96-19',
  'level': 'A',
  'name': '1e48810bb4',
  'natScore': 75,
  'order': 1,
  'subject': 'Business'},
 {'grade': 'AD1',
  'index': '97-2',
  'level': 'A',
  'name': '55cc935bd4',
  'natScore': 11,
  'order': 1,
  'subject': 'Irish'},
 {'grade': 'AD1',
  'index': '97-3',
  'level': 'A',
  'name': '55cc935bd4',
  'natScore': 80,
  'order': 1,
  'subject': 'English'},
 {'grade': 'GB3',
  'index': '97-4',
  'level': 'G',
  'name': '55cc935bd4',
  'natScore': 15,
  'order': 1,
  'subject': 'Maths'},
 {'grade': 'AB3',
  'index': '97-5',
  'level': 'A',
  'name': '55cc935bd4',
  'natScore': 79,
  'order': 1,
  'subject': 'History'},
 {'grade': 'AC2',
  'index': '97-6',
  'level': 'A',
  'name': '55cc935bd4',
  'natScore': 76,
  'order': 1,
  'subject': 'Geography'},
 {'grade': 'AD1',
  'index': '97-8',
  'level': 'A',
  'name': '55cc935bd4',
  'natScore': 90,
  'order': 1,
  'subject': 'German'},
 {'grade': 'AD1',
  'index': '97-15',
  'level': 'A',
  'name': '55cc935bd4',
  'natScore': 100,
  'order': 1,
  'subject': 'Biology'},
 {'grade': 'AD2',
  'index': '98-2',
  'level': 'A',
  'name': '0138cd1ab5',
  'natScore': 50,
  'order': 1,
  'subject': 'Irish'},
 {'grade': 'GA2',
  'index': '98-3',
  'level': 'G',
  'name': '0138cd1ab5',
  'natScore': 93,
  'order': 1,
  'subject': 'English'},
 {'grade': 'AD1',
  'index': '98-4',
  'level': 'A',
  'name': '0138cd1ab5',
  'natScore': 27,
  'order': 1,
  'subject': 'Maths'},
 {'grade': 'AC1',
  'index': '98-6',
  'level': 'A',
  'name': '0138cd1ab5',
  'natScore': 35,
  'order': 1,
  'subject': 'Geography'},
 {'grade': 'AB3',
  'index': '98-14',
  'level': 'A',
  'name': '0138cd1ab5',
  'natScore': 42,
  'order': 1,
  'subject': 'Ag. Science'},
 {'grade': 'AB3',
  'index': '98-15',
  'level': 'A',
  'name': '0138cd1ab5',
  'natScore': 1,
  'order': 1,
  'subject': 'Biology'},
 {'grade': 'AC3',
  'index': '98-19',
  'level': 'A',
  'name': '0138cd1ab5',
  'natScore': 25,
  'order': 1,
  'subject': 'Business'},
 {'grade': 'GB3',
  'index': '99-3',
  'level': 'G',
  'name': '0c8b1fd8ec',
  'natScore': 46,
  'order': 1,
  'subject': 'English'},
 {'grade': 'GD2',
  'index': '99-4',
  'level': 'G',
  'name': '0c8b1fd8ec',
  'natScore': 65,
  'order': 1,
  'subject': 'Maths'},
 {'grade': 'AD3',
  'index': '99-6',
  'level': 'A',
  'name': '0c8b1fd8ec',
  'natScore': 8,
  'order': 1,
  'subject': 'Geography'},
 {'grade': 'AD2',
  'index': '99-10',
  'level': 'A',
  'name': '0c8b1fd8ec',
  'natScore': 77,
  'order': 1,
  'subject': 'Art'},
 {'grade': 'AC3',
  'index': '99-16',
  'level': 'A',
  'name': '0c8b1fd8ec',
  'natScore': 22,
  'order': 1,
  'subject': 'Engineering'},
 {'grade': 'AC1',
  'index': '99-17',
  'level': 'A',
  'name': '0c8b1fd8ec',
  'natScore': 61,
  'order': 1,
  'subject': 'Construction Studies'},
 {'grade': 'GB3',
  'index': '100-2',
  'level': 'G',
  'name': '2794821b62',
  'natScore': 68,
  'order': 1,
  'subject': 'Irish'},
 {'grade': 'AC3',
  'index': '100-3',
  'level': 'A',
  'name': '2794821b62',
  'natScore': 93,
  'order': 1,
  'subject': 'English'},
 {'grade': 'AC3',
  'index': '100-4',
  'level': 'A',
  'name': '2794821b62',
  'natScore': 91,
  'order': 1,
  'subject': 'Maths'},
 {'grade': 'AB3',
  'index': '100-6',
  'level': 'A',
  'name': '2794821b62',
  'natScore': 82,
  'order': 1,
  'subject': 'Geography'},
 {'grade': 'AD1',
  'index': '100-8',
  'level': 'A',
  'name': '2794821b62',
  'natScore': 9,
  'order': 1,
  'subject': 'German'},
 {'grade': 'AB3',
  'index': '100-12',
  'level': 'A',
  'name': '2794821b62',
  'natScore': 14,
  'order': 1,
  'subject': 'Physics'},
 {'grade': 'AD1',
  'index': '100-13',
  'level': 'A',
  'name': '2794821b62',
  'natScore': 75,
  'order': 1,
  'subject': 'Chemistry'},
 {'grade': 'AA2',
  'index': '100-17',
  'level': 'A',
  'name': '2794821b62',
  'natScore': 78,
  'order': 1,
  'subject': 'Construction Studies'},
 {'grade': 'AC2',
  'index': '101-2',
  'level': 'A',
  'name': 'a3116c6342',
  'natScore': 91,
  'order': 1,
  'subject': 'Irish'},
 {'grade': 'GA2',
  'index': '101-3',
  'level': 'G',
  'name': 'a3116c6342',
  'natScore': 31,
  'order': 1,
  'subject': 'English'},
 {'grade': 'AC2',
  'index': '101-4',
  'level': 'A',
  'name': 'a3116c6342',
  'natScore': 37,
  'order': 1,
  'subject': 'Maths'},
 {'grade': 'AC3',
  'index': '101-8',
  'level': 'A',
  'name': 'a3116c6342',
  'natScore': 83,
  'order': 1,
  'subject': 'German'},
 {'grade': 'AC2',
  'index': '101-14',
  'level': 'A',
  'name': 'a3116c6342',
  'natScore': 52,
  'order': 1,
  'subject': 'Ag. Science'},
 {'grade': 'AC1',
  'index': '101-15',
  'level': 'A',
  'name': 'a3116c6342',
  'natScore': 94,
  'order': 1,
  'subject': 'Biology'},
 {'grade': 'AC1',
  'index': '101-19',
  'level': 'A',
  'name': 'a3116c6342',
  'natScore': 13,
  'order': 1,
  'subject': 'Business'},
 {'grade': 'GC3',
  'index': '102-3',
  'level': 'G',
  'name': 'c46a3353f0',
  'natScore': 2,
  'order': 1,
  'subject': 'English'},
 {'grade': 'GE',
  'index': '102-6',
  'level': 'G',
  'name': 'c46a3353f0',
  'natScore': 49,
  'order': 1,
  'subject': 'Geography'},
 {'grade': 'GD2',
  'index': '102-10',
  'level': 'G',
  'name': 'c46a3353f0',
  'natScore': 69,
  'order': 1,
  'subject': 'Art'},
 {'grade': 'AC3',
  'index': '103-2',
  'level': 'A',
  'name': '186bf34fa0',
  'natScore': 92,
  'order': 1,
  'subject': 'Irish'},
 {'grade': 'AC1',
  'index': '103-3',
  'level': 'A',
  'name': '186bf34fa0',
  'natScore': 10,
  'order': 1,
  'subject': 'English'},
 {'grade': 'GD2',
  'index': '103-4',
  'level': 'G',
  'name': '186bf34fa0',
  'natScore': 81,
  'order': 1,
  'subject': 'Maths'},
 {'grade': 'AD1',
  'index': '103-5',
  'level': 'A',
  'name': '186bf34fa0',
  'natScore': 43,
  'order': 1,
  'subject': 'History'},
 {'grade': 'AC2',
  'index': '103-6',
  'level': 'A',
  'name': '186bf34fa0',
  'natScore': 95,
  'order': 1,
  'subject': 'Geography'},
 {'grade': 'AA2',
  'index': '103-9',
  'level': 'A',
  'name': '186bf34fa0',
  'natScore': 65,
  'order': 1,
  'subject': 'Spanish'},
 {'grade': 'AD2',
  'index': '103-19',
  'level': 'A',
  'name': '186bf34fa0',
  'natScore': 30,
  'order': 1,
  'subject': 'Business'},
 {'grade': 'GE',
  'index': '104-2',
  'level': 'G',
  'name': '26d1555499',
  'natScore': 99,
  'order': 1,
  'subject': 'Irish'},
 {'grade': 'GC1',
  'index': '104-3',
  'level': 'G',
  'name': '26d1555499',
  'natScore': 43,
  'order': 1,
  'subject': 'English'},
 {'grade': 'GD3',
  'index': '104-4',
  'level': 'G',
  'name': '26d1555499',
  'natScore': 86,
  'order': 1,
  'subject': 'Maths'},
 {'grade': 'GC2',
  'index': '104-6',
  'level': 'G',
  'name': '26d1555499',
  'natScore': 86,
  'order': 1,
  'subject': 'Geography'},
 {'grade': 'GD2',
  'index': '104-10',
  'level': 'G',
  'name': '26d1555499',
  'natScore': 32,
  'order': 1,
  'subject': 'Art'},
 {'grade': 'GD2',
  'index': '104-19',
  'level': 'G',
  'name': '26d1555499',
  'natScore': 69,
  'order': 1,
  'subject': 'Business'},
 {'grade': 'GC1',
  'index': '105-3',
  'level': 'G',
  'name': '0eb22fbf77',
  'natScore': 16,
  'order': 1,
  'subject': 'English'},
 {'grade': 'GD2',
  'index': '105-4',
  'level': 'G',
  'name': '0eb22fbf77',
  'natScore': 44,
  'order': 1,
  'subject': 'Maths'},
 {'grade': 'GD1',
  'index': '105-6',
  'level': 'G',
  'name': '0eb22fbf77',
  'natScore': 84,
  'order': 1,
  'subject': 'Geography'},
 {'grade': 'GC2',
  'index': '105-9',
  'level': 'G',
  'name': '0eb22fbf77',
  'natScore': 27,
  'order': 1,
  'subject': 'Spanish'},
 {'grade': 'GC3',
  'index': '105-15',
  'level': 'G',
  'name': '0eb22fbf77',
  'natScore': 15,
  'order': 1,
  'subject': 'Biology'},
 {'grade': 'AB1',
  'index': '106-2',
  'level': 'A',
  'name': '42a104fd04',
  'natScore': 40,
  'order': 1,
  'subject': 'Irish'},
 {'grade': 'GA1',
  'index': '106-3',
  'level': 'G',
  'name': '42a104fd04',
  'natScore': 19,
  'order': 1,
  'subject': 'English'},
 {'grade': 'AB1',
  'index': '106-4',
  'level': 'A',
  'name': '42a104fd04',
  'natScore': 38,
  'order': 1,
  'subject': 'Maths'},
 {'grade': 'AB1',
  'index': '106-5',
  'level': 'A',
  'name': '42a104fd04',
  'natScore': 69,
  'order': 1,
  'subject': 'History'},
 {'grade': 'AB1',
  'index': '106-6',
  'level': 'A',
  'name': '42a104fd04',
  'natScore': 88,
  'order': 1,
  'subject': 'Geography'},
 {'grade': 'AB2',
  'index': '106-13',
  'level': 'A',
  'name': '42a104fd04',
  'natScore': 28,
  'order': 1,
  'subject': 'Chemistry'},
 {'grade': 'AA2',
  'index': '106-20',
  'level': 'A',
  'name': '42a104fd04',
  'natScore': 22,
  'order': 1,
  'subject': 'Economics'},
 {'grade': 'AC1',
  'index': '107-2',
  'level': 'A',
  'name': '6e91943b25',
  'natScore': 95,
  'order': 1,
  'subject': 'Irish'},
 {'grade': 'AC2',
  'index': '107-3',
  'level': 'A',
  'name': '6e91943b25',
  'natScore': 8,
  'order': 1,
  'subject': 'English'},
 {'grade': 'GD2',
  'index': '107-4',
  'level': 'G',
  'name': '6e91943b25',
  'natScore': 16,
  'order': 1,
  'subject': 'Maths'},
 {'grade': 'AC2',
  'index': '107-9',
  'level': 'A',
  'name': '6e91943b25',
  'natScore': 72,
  'order': 1,
  'subject': 'Spanish'},
 {'grade': 'AD2',
  'index': '107-14',
  'level': 'A',
  'name': '6e91943b25',
  'natScore': 67,
  'order': 1,
  'subject': 'Ag. Science'},
 {'grade': 'GB1',
  'index': '107-20',
  'level': 'G',
  'name': '6e91943b25',
  'natScore': 72,
  'order': 1,
  'subject': 'Economics'},
 {'grade': 'AC2',
  'index': '107-21',
  'level': 'A',
  'name': '6e91943b25',
  'natScore': 58,
  'order': 1,
  'subject': 'Music'},
 {'grade': 'GC3',
  'index': '108-2',
  'level': 'G',
  'name': 'b40b9ab847',
  'natScore': 68,
  'order': 1,
  'subject': 'Irish'},
 {'grade': 'AB3',
  'index': '108-3',
  'level': 'A',
  'name': 'b40b9ab847',
  'natScore': 42,
  'order': 1,
  'subject': 'English'},
 {'grade': 'GC1',
  'index': '108-4',
  'level': 'G',
  'name': 'b40b9ab847',
  'natScore': 52,
  'order': 1,
  'subject': 'Maths'},
 {'grade': 'AC3',
  'index': '108-6',
  'level': 'A',
  'name': 'b40b9ab847',
  'natScore': 48,
  'order': 1,
  'subject': 'Geography'},
 {'grade': 'AD3',
  'index': '108-14',
  'level': 'A',
  'name': 'b40b9ab847',
  'natScore': 36,
  'order': 1,
  'subject': 'Ag. Science'},
 {'grade': 'AD2',
  'index': '108-15',
  'level': 'A',
  'name': 'b40b9ab847',
  'natScore': 78,
  'order': 1,
  'subject': 'Biology'},
 {'grade': 'AD2',
  'index': '108-19',
  'level': 'A',
  'name': 'b40b9ab847',
  'natScore': 39,
  'order': 1,
  'subject': 'Business'},
 {'grade': 'GC2',
  'index': '109-2',
  'level': 'G',
  'name': 'e9075e1b6e',
  'natScore': 25,
  'order': 1,
  'subject': 'Irish'},
 {'grade': 'GB1',
  'index': '109-3',
  'level': 'G',
  'name': 'e9075e1b6e',
  'natScore': 17,
  'order': 1,
  'subject': 'English'},
 {'grade': 'GC2',
  'index': '109-6',
  'level': 'G',
  'name': 'e9075e1b6e',
  'natScore': 33,
  'order': 1,
  'subject': 'Geography'},
 {'grade': 'GD2',
  'index': '109-15',
  'level': 'G',
  'name': 'e9075e1b6e',
  'natScore': 15,
  'order': 1,
  'subject': 'Biology'},
 {'grade': 'AC3',
  'index': '109-16',
  'level': 'A',
  'name': 'e9075e1b6e',
  'natScore': 66,
  'order': 1,
  'subject': 'Engineering'},
 {'grade': 'GA2',
  'index': '110-2',
  'level': 'G',
  'name': '84e6d98129',
  'natScore': 76,
  'order': 1,
  'subject': 'Irish'},
 {'grade': 'AA1',
  'index': '110-3',
  'level': 'A',
  'name': '84e6d98129',
  'natScore': 57,
  'order': 1,
  'subject': 'English'},
 {'grade': 'AB2',
  'index': '110-4',
  'level': 'A',
  'name': '84e6d98129',
  'natScore': 99,
  'order': 1,
  'subject': 'Maths'},
 {'grade': 'AA1',
  'index': '110-5',
  'level': 'A',
  'name': '84e6d98129',
  'natScore': 8,
  'order': 1,
  'subject': 'History'},
 {'grade': 'AA2',
  'index': '110-6',
  'level': 'A',
  'name': '84e6d98129',
  'natScore': 2,
  'order': 1,
  'subject': 'Geography'},
 {'grade': 'AA1',
  'index': '110-13',
  'level': 'A',
  'name': '84e6d98129',
  'natScore': 81,
  'order': 1,
  'subject': 'Chemistry'},
 {'grade': 'AA1',
  'index': '110-14',
  'level': 'A',
  'name': '84e6d98129',
  'natScore': 27,
  'order': 1,
  'subject': 'Ag. Science'},
 {'grade': 'AA1',
  'index': '110-15',
  'level': 'A',
  'name': '84e6d98129',
  'natScore': 84,
  'order': 1,
  'subject': 'Biology'},
 {'grade': 'GC1',
  'index': '111-2',
  'level': 'G',
  'name': '31f515483d',
  'natScore': 68,
  'order': 1,
  'subject': 'Irish'},
 {'grade': 'AC3',
  'index': '111-3',
  'level': 'A',
  'name': '31f515483d',
  'natScore': 11,
  'order': 1,
  'subject': 'English'},
 {'grade': 'GC2',
  'index': '111-4',
  'level': 'G',
  'name': '31f515483d',
  'natScore': 96,
  'order': 1,
  'subject': 'Maths'},
 {'grade': 'AB3',
  'index': '111-6',
  'level': 'A',
  'name': '31f515483d',
  'natScore': 33,
  'order': 1,
  'subject': 'Geography'},
 {'grade': 'AC2',
  'index': '111-14',
  'level': 'A',
  'name': '31f515483d',
  'natScore': 20,
  'order': 1,
  'subject': 'Ag. Science'},
 {'grade': 'AB2',
  'index': '111-19',
  'level': 'A',
  'name': '31f515483d',
  'natScore': 56,
  'order': 1,
  'subject': 'Business'},
 {'grade': 'AB3',
  'index': '111-22',
  'level': 'A',
  'name': '31f515483d',
  'natScore': 87,
  'order': 1,
  'subject': 'Home Ec.'},
 {'grade': 'AA1',
  'index': '112-2',
  'level': 'A',
  'name': 'b7fe47d680',
  'natScore': 80,
  'order': 1,
  'subject': 'Irish'},
 {'grade': 'AA2',
  'index': '112-3',
  'level': 'A',
  'name': 'b7fe47d680',
  'natScore': 90,
  'order': 1,
  'subject': 'English'},
 {'grade': 'AA1',
  'index': '112-4',
  'level': 'A',
  'name': 'b7fe47d680',
  'natScore': 51,
  'order': 1,
  'subject': 'Maths'},
 {'grade': 'AA2',
  'index': '112-8',
  'level': 'A',
  'name': 'b7fe47d680',
  'natScore': 81,
  'order': 1,
  'subject': 'German'},
 {'grade': 'AA1',
  'index': '112-12',
  'level': 'A',
  'name': 'b7fe47d680',
  'natScore': 45,
  'order': 1,
  'subject': 'Physics'},
 {'grade': 'AA1',
  'index': '112-15',
  'level': 'A',
  'name': 'b7fe47d680',
  'natScore': 30,
  'order': 1,
  'subject': 'Biology'},
 {'grade': 'AA1',
  'index': '112-16',
  'level': 'A',
  'name': 'b7fe47d680',
  'natScore': 93,
  'order': 1,
  'subject': 'Engineering'},
 {'grade': 'AC2',
  'index': '113-2',
  'level': 'A',
  'name': '24a07a2cb3',
  'natScore': 85,
  'order': 1,
  'subject': 'Irish'},
 {'grade': 'AD1',
  'index': '113-3',
  'level': 'A',
  'name': '24a07a2cb3',
  'natScore': 91,
  'order': 1,
  'subject': 'English'},
 {'grade': 'GB3',
  'index': '113-4',
  'level': 'G',
  'name': '24a07a2cb3',
  'natScore': 3,
  'order': 1,
  'subject': 'Maths'},
 {'grade': 'AC3',
  'index': '113-8',
  'level': 'A',
  'name': '24a07a2cb3',
  'natScore': 47,
  'order': 1,
  'subject': 'German'},
 {'grade': 'AD2',
  'index': '113-15',
  'level': 'A',
  'name': '24a07a2cb3',
  'natScore': 58,
  'order': 1,
  'subject': 'Biology'},
 {'grade': 'GC1',
  'index': '113-20',
  'level': 'G',
  'name': '24a07a2cb3',
  'natScore': 4,
  'order': 1,
  'subject': 'Economics'},
 {'grade': 'AB3',
  'index': '113-21',
  'level': 'A',
  'name': '24a07a2cb3',
  'natScore': 29,
  'order': 1,
  'subject': 'Music'},
 {'grade': 'AA1',
  'index': '114-2',
  'level': 'A',
  'name': '9d02bb614e',
  'natScore': 67,
  'order': 1,
  'subject': 'Irish'},
 {'grade': 'AC2',
  'index': '114-3',
  'level': 'A',
  'name': '9d02bb614e',
  'natScore': 23,
  'order': 1,
  'subject': 'English'},
 {'grade': 'AC1',
  'index': '114-4',
  'level': 'A',
  'name': '9d02bb614e',
  'natScore': 100,
  'order': 1,
  'subject': 'Maths'},
 {'grade': 'AC1',
  'index': '114-6',
  'level': 'A',
  'name': '9d02bb614e',
  'natScore': 19,
  'order': 1,
  'subject': 'Geography'},
 {'grade': 'GB1',
  'index': '114-8',
  'level': 'G',
  'name': '9d02bb614e',
  'natScore': 72,
  'order': 1,
  'subject': 'German'},
 {'grade': 'AB1',
  'index': '114-15',
  'level': 'A',
  'name': '9d02bb614e',
  'natScore': 35,
  'order': 1,
  'subject': 'Biology'},
 {'grade': 'AB3',
  'index': '114-18',
  'level': 'A',
  'name': '9d02bb614e',
  'natScore': 47,
  'order': 1,
  'subject': 'Accounting'},
 {'grade': 'GD2',
  'index': '115-2',
  'level': 'G',
  'name': 'd4c76d1adf',
  'natScore': 0,
  'order': 1,
  'subject': 'Irish'},
 {'grade': 'GC1',
  'index': '115-3',
  'level': 'G',
  'name': 'd4c76d1adf',
  'natScore': 56,
  'order': 1,
  'subject': 'English'},
 {'grade': 'GD1',
  'index': '115-4',
  'level': 'G',
  'name': 'd4c76d1adf',
  'natScore': 55,
  'order': 1,
  'subject': 'Maths'},
 {'grade': 'AE',
  'index': '115-6',
  'level': 'A',
  'name': 'd4c76d1adf',
  'natScore': 54,
  'order': 1,
  'subject': 'Geography'},
 {'grade': 'GD2',
  'index': '115-14',
  'level': 'G',
  'name': 'd4c76d1adf',
  'natScore': 26,
  'order': 1,
  'subject': 'Ag. Science'},
 {'grade': 'AE',
  'index': '115-19',
  'level': 'A',
  'name': 'd4c76d1adf',
  'natScore': 84,
  'order': 1,
  'subject': 'Business'},
 {'grade': 'AD2',
  'index': '115-22',
  'level': 'A',
  'name': 'd4c76d1adf',
  'natScore': 15,
  'order': 1,
  'subject': 'Home Ec.'},
 {'grade': 'GC3',
  'index': '116-2',
  'level': 'G',
  'name': 'f4f03bbca0',
  'natScore': 91,
  'order': 1,
  'subject': 'Irish'},
 {'grade': 'GC1',
  'index': '116-3',
  'level': 'G',
  'name': 'f4f03bbca0',
  'natScore': 72,
  'order': 1,
  'subject': 'English'},
 {'grade': 'GE',
  'index': '116-4',
  'level': 'G',
  'name': 'f4f03bbca0',
  'natScore': 82,
  'order': 1,
  'subject': 'Maths'},
 {'grade': 'GD2',
  'index': '116-6',
  'level': 'G',
  'name': 'f4f03bbca0',
  'natScore': 34,
  'order': 1,
  'subject': 'Geography'},
 {'grade': 'GE',
  'index': '116-14',
  'level': 'G',
  'name': 'f4f03bbca0',
  'natScore': 1,
  'order': 1,
  'subject': 'Ag. Science'},
 {'grade': 'GE',
  'index': '116-19',
  'level': 'G',
  'name': 'f4f03bbca0',
  'natScore': 88,
  'order': 1,
  'subject': 'Business'},
 {'grade': 'GD3',
  'index': '116-22',
  'level': 'G',
  'name': 'f4f03bbca0',
  'natScore': 27,
  'order': 1,
  'subject': 'Home Ec.'},
 {'grade': 'AD2',
  'index': '117-2',
  'level': 'A',
  'name': 'c887dd2bc8',
  'natScore': 0,
  'order': 1,
  'subject': 'Irish'},
 {'grade': 'AC2',
  'index': '117-3',
  'level': 'A',
  'name': 'c887dd2bc8',
  'natScore': 6,
  'order': 1,
  'subject': 'English'},
 {'grade': 'AD1',
  'index': '117-4',
  'level': 'A',
  'name': 'c887dd2bc8',
  'natScore': 63,
  'order': 1,
  'subject': 'Maths'},
 {'grade': 'AC2',
  'index': '117-5',
  'level': 'A',
  'name': 'c887dd2bc8',
  'natScore': 57,
  'order': 1,
  'subject': 'History'},
 {'grade': 'AC1',
  'index': '117-6',
  'level': 'A',
  'name': 'c887dd2bc8',
  'natScore': 28,
  'order': 1,
  'subject': 'Geography'},
 {'grade': 'AC3',
  'index': '117-8',
  'level': 'A',
  'name': 'c887dd2bc8',
  'natScore': 39,
  'order': 1,
  'subject': 'German'},
 {'grade': 'AC3',
  'index': '117-20',
  'level': 'A',
  'name': 'c887dd2bc8',
  'natScore': 66,
  'order': 1,
  'subject': 'Economics'},
 {'grade': 'AA2',
  'index': '118-2',
  'level': 'A',
  'name': '6b99f181bb',
  'natScore': 87,
  'order': 1,
  'subject': 'Irish'},
 {'grade': 'AA2',
  'index': '118-3',
  'level': 'A',
  'name': '6b99f181bb',
  'natScore': 91,
  'order': 1,
  'subject': 'English'},
 {'grade': 'AB1',
  'index': '118-4',
  'level': 'A',
  'name': '6b99f181bb',
  'natScore': 10,
  'order': 1,
  'subject': 'Maths'},
 {'grade': 'AA2',
  'index': '118-7',
  'level': 'A',
  'name': '6b99f181bb',
  'natScore': 3,
  'order': 1,
  'subject': 'French'},
 {'grade': 'AA2',
  'index': '118-15',
  'level': 'A',
  'name': '6b99f181bb',
  'natScore': 100,
  'order': 1,
  'subject': 'Biology'},
 {'grade': 'AA1',
  'index': '118-18',
  'level': 'A',
  'name': '6b99f181bb',
  'natScore': 41,
  'order': 1,
  'subject': 'Accounting'},
 {'grade': 'AA2',
  'index': '118-21',
  'level': 'A',
  'name': '6b99f181bb',
  'natScore': 6,
  'order': 1,
  'subject': 'Music'},
 {'grade': 'GC3',
  'index': '119-2',
  'level': 'G',
  'name': '13c4c98f0b',
  'natScore': 51,
  'order': 1,
  'subject': 'Irish'},
 {'grade': 'GB3',
  'index': '119-3',
  'level': 'G',
  'name': '13c4c98f0b',
  'natScore': 46,
  'order': 1,
  'subject': 'English'},
 {'grade': 'GD2',
  'index': '119-4',
  'level': 'G',
  'name': '13c4c98f0b',
  'natScore': 46,
  'order': 1,
  'subject': 'Maths'},
 {'grade': 'GD2',
  'index': '119-7',
  'level': 'G',
  'name': '13c4c98f0b',
  'natScore': 83,
  'order': 1,
  'subject': 'French'},
 {'grade': 'GC2',
  'index': '119-14',
  'level': 'G',
  'name': '13c4c98f0b',
  'natScore': 12,
  'order': 1,
  'subject': 'Ag. Science'},
 {'grade': 'GB3',
  'index': '119-15',
  'level': 'G',
  'name': '13c4c98f0b',
  'natScore': 79,
  'order': 1,
  'subject': 'Biology'},
 {'grade': 'GC1',
  'index': '119-21',
  'level': 'G',
  'name': '13c4c98f0b',
  'natScore': 71,
  'order': 1,
  'subject': 'Music'},
 {'grade': 'GB3',
  'index': '120-2',
  'level': 'G',
  'name': '0eeb31fdef',
  'natScore': 18,
  'order': 1,
  'subject': 'Irish'},
 {'grade': 'AC2',
  'index': '120-3',
  'level': 'A',
  'name': '0eeb31fdef',
  'natScore': 59,
  'order': 1,
  'subject': 'English'},
 {'grade': 'GB3',
  'index': '120-4',
  'level': 'G',
  'name': '0eeb31fdef',
  'natScore': 98,
  'order': 1,
  'subject': 'Maths'},
 {'grade': 'AB2',
  'index': '120-7',
  'level': 'A',
  'name': '0eeb31fdef',
  'natScore': 94,
  'order': 1,
  'subject': 'French'},
 {'grade': 'AC2',
  'index': '120-15',
  'level': 'A',
  'name': '0eeb31fdef',
  'natScore': 60,
  'order': 1,
  'subject': 'Biology'},
 {'grade': 'AC2',
  'index': '120-18',
  'level': 'A',
  'name': '0eeb31fdef',
  'natScore': 29,
  'order': 1,
  'subject': 'Accounting'},
 {'grade': 'AB3',
  'index': '120-19',
  'level': 'A',
  'name': '0eeb31fdef',
  'natScore': 98,
  'order': 1,
  'subject': 'Business'},
 {'grade': 'GB2',
  'index': '121-2',
  'level': 'G',
  'name': '7f2d287c27',
  'natScore': 85,
  'order': 1,
  'subject': 'Irish'},
 {'grade': 'AB2',
  'index': '121-3',
  'level': 'A',
  'name': '7f2d287c27',
  'natScore': 0,
  'order': 1,
  'subject': 'English'},
 {'grade': 'AC2',
  'index': '121-4',
  'level': 'A',
  'name': '7f2d287c27',
  'natScore': 86,
  'order': 1,
  'subject': 'Maths'},
 {'grade': 'AC1',
  'index': '121-6',
  'level': 'A',
  'name': '7f2d287c27',
  'natScore': 90,
  'order': 1,
  'subject': 'Geography'},
 {'grade': 'AB2',
  'index': '121-16',
  'level': 'A',
  'name': '7f2d287c27',
  'natScore': 15,
  'order': 1,
  'subject': 'Engineering'},
 {'grade': 'AA2',
  'index': '121-17',
  'level': 'A',
  'name': '7f2d287c27',
  'natScore': 61,
  'order': 1,
  'subject': 'Construction Studies'},
 {'grade': 'AC3',
  'index': '121-20',
  'level': 'A',
  'name': '7f2d287c27',
  'natScore': 68,
  'order': 1,
  'subject': 'Economics'},
 {'grade': 'AC2',
  'index': '122-2',
  'level': 'A',
  'name': 'c8b6339010',
  'natScore': 18,
  'order': 1,
  'subject': 'Irish'},
 {'grade': 'AB2',
  'index': '122-3',
  'level': 'A',
  'name': 'c8b6339010',
  'natScore': 71,
  'order': 1,
  'subject': 'English'},
 {'grade': 'GB2',
  'index': '122-4',
  'level': 'G',
  'name': 'c8b6339010',
  'natScore': 83,
  'order': 1,
  'subject': 'Maths'},
 {'grade': 'AC1',
  'index': '122-5',
  'level': 'A',
  'name': 'c8b6339010',
  'natScore': 63,
  'order': 1,
  'subject': 'History'},
 {'grade': 'AC2',
  'index': '122-6',
  'level': 'A',
  'name': 'c8b6339010',
  'natScore': 26,
  'order': 1,
  'subject': 'Geography'},
 {'grade': 'AB3',
  'index': '122-21',
  'level': 'A',
  'name': 'c8b6339010',
  'natScore': 42,
  'order': 1,
  'subject': 'Music'},
 {'grade': 'AC1',
  'index': '122-22',
  'level': 'A',
  'name': 'c8b6339010',
  'natScore': 17,
  'order': 1,
  'subject': 'Home Ec.'},
 {'grade': 'GC1',
  'index': '123-2',
  'level': 'G',
  'name': 'd90210c800',
  'natScore': 17,
  'order': 1,
  'subject': 'Irish'},
 {'grade': 'AD1',
  'index': '123-3',
  'level': 'A',
  'name': 'd90210c800',
  'natScore': 48,
  'order': 1,
  'subject': 'English'},
 {'grade': 'GC3',
  'index': '123-4',
  'level': 'G',
  'name': 'd90210c800',
  'natScore': 74,
  'order': 1,
  'subject': 'Maths'},
 {'grade': 'AC1',
  'index': '123-6',
  'level': 'A',
  'name': 'd90210c800',
  'natScore': 23,
  'order': 1,
  'subject': 'Geography'},
 {'grade': 'AD3',
  'index': '123-14',
  'level': 'A',
  'name': 'd90210c800',
  'natScore': 98,
  'order': 1,
  'subject': 'Ag. Science'},
 {'grade': 'AD3',
  'index': '123-15',
  'level': 'A',
  'name': 'd90210c800',
  'natScore': 88,
  'order': 1,
  'subject': 'Biology'},
 {'grade': 'AE',
  'index': '123-18',
  'level': 'A',
  'name': 'd90210c800',
  'natScore': 45,
  'order': 1,
  'subject': 'Accounting'},
 {'grade': 'AB2',
  'index': '123-19',
  'level': 'A',
  'name': 'd90210c800',
  'natScore': 74,
  'order': 1,
  'subject': 'Business'},
 {'grade': 'GB1',
  'index': '124-2',
  'level': 'G',
  'name': 'cea6ee3bfd',
  'natScore': 58,
  'order': 1,
  'subject': 'Irish'},
 {'grade': 'AA2',
  'index': '124-3',
  'level': 'A',
  'name': 'cea6ee3bfd',
  'natScore': 55,
  'order': 1,
  'subject': 'English'},
 {'grade': 'AB1',
  'index': '124-4',
  'level': 'A',
  'name': 'cea6ee3bfd',
  'natScore': 58,
  'order': 1,
  'subject': 'Maths'},
 {'grade': 'AA2',
  'index': '124-7',
  'level': 'A',
  'name': 'cea6ee3bfd',
  'natScore': 48,
  'order': 1,
  'subject': 'French'},
 {'grade': 'AA2',
  'index': '124-12',
  'level': 'A',
  'name': 'cea6ee3bfd',
  'natScore': 77,
  'order': 1,
  'subject': 'Physics'},
 {'grade': 'AB1',
  'index': '124-13',
  'level': 'A',
  'name': 'cea6ee3bfd',
  'natScore': 71,
  'order': 1,
  'subject': 'Chemistry'},
 {'grade': 'AA1',
  'index': '124-15',
  'level': 'A',
  'name': 'cea6ee3bfd',
  'natScore': 35,
  'order': 1,
  'subject': 'Biology'},
 {'grade': 'AA2',
  'index': '124-21',
  'level': 'A',
  'name': 'cea6ee3bfd',
  'natScore': 82,
  'order': 1,
  'subject': 'Music'},
 {'grade': 'AA1',
  'index': '125-2',
  'level': 'A',
  'name': 'e96d46daca',
  'natScore': 29,
  'order': 1,
  'subject': 'Irish'},
 {'grade': 'AA1',
  'index': '125-3',
  'level': 'A',
  'name': 'e96d46daca',
  'natScore': 13,
  'order': 1,
  'subject': 'English'},
 {'grade': 'AB1',
  'index': '125-4',
  'level': 'A',
  'name': 'e96d46daca',
  'natScore': 66,
  'order': 1,
  'subject': 'Maths'},
 {'grade': 'AA2',
  'index': '125-8',
  'level': 'A',
  'name': 'e96d46daca',
  'natScore': 5,
  'order': 1,
  'subject': 'German'},
 {'grade': 'AB3',
  'index': '125-11',
  'level': 'A',
  'name': 'e96d46daca',
  'natScore': 73,
  'order': 1,
  'subject': 'Applied Maths'},
 {'grade': 'AA2',
  'index': '125-12',
  'level': 'A',
  'name': 'e96d46daca',
  'natScore': 54,
  'order': 1,
  'subject': 'Physics'},
 {'grade': 'AA2',
  'index': '125-15',
  'level': 'A',
  'name': 'e96d46daca',
  'natScore': 60,
  'order': 1,
  'subject': 'Biology'},
 {'grade': 'AB1',
  'index': '125-16',
  'level': 'A',
  'name': 'e96d46daca',
  'natScore': 40,
  'order': 1,
  'subject': 'Engineering'},
 {'grade': 'GB2',
  'index': '126-3',
  'level': 'G',
  'name': '5e9c467710',
  'natScore': 84,
  'order': 1,
  'subject': 'English'},
 {'grade': 'GB2',
  'index': '126-4',
  'level': 'G',
  'name': '5e9c467710',
  'natScore': 58,
  'order': 1,
  'subject': 'Maths'},
 {'grade': 'AD2',
  'index': '126-6',
  'level': 'A',
  'name': '5e9c467710',
  'natScore': 12,
  'order': 1,
  'subject': 'Geography'},
 {'grade': 'AD2',
  'index': '126-10',
  'level': 'A',
  'name': '5e9c467710',
  'natScore': 2,
  'order': 1,
  'subject': 'Art'},
 {'grade': 'GC1',
  'index': '126-14',
  'level': 'G',
  'name': '5e9c467710',
  'natScore': 60,
  'order': 1,
  'subject': 'Ag. Science'},
 {'grade': 'AC1',
  'index': '126-16',
  'level': 'A',
  'name': '5e9c467710',
  'natScore': 46,
  'order': 1,
  'subject': 'Engineering'},
 {'grade': 'AC2',
  'index': '127-2',
  'level': 'A',
  'name': '3a732bc011',
  'natScore': 38,
  'order': 1,
  'subject': 'Irish'},
 {'grade': 'AC3',
  'index': '127-3',
  'level': 'A',
  'name': '3a732bc011',
  'natScore': 65,
  'order': 1,
  'subject': 'English'},
 {'grade': 'GB3',
  'index': '127-4',
  'level': 'G',
  'name': '3a732bc011',
  'natScore': 41,
  'order': 1,
  'subject': 'Maths'},
 {'grade': 'AC3',
  'index': '127-7',
  'level': 'A',
  'name': '3a732bc011',
  'natScore': 45,
  'order': 1,
  'subject': 'French'},
 {'grade': 'AB2',
  'index': '127-14',
  'level': 'A',
  'name': '3a732bc011',
  'natScore': 59,
  'order': 1,
  'subject': 'Ag. Science'},
 {'grade': 'AC2',
  'index': '127-15',
  'level': 'A',
  'name': '3a732bc011',
  'natScore': 1,
  'order': 1,
  'subject': 'Biology'},
 {'grade': 'AC1',
  'index': '127-19',
  'level': 'A',
  'name': '3a732bc011',
  'natScore': 36,
  'order': 1,
  'subject': 'Business'},
 {'grade': 'GC2',
  'index': '128-3',
  'level': 'G',
  'name': 'c0a78d6cba',
  'natScore': 30,
  'order': 1,
  'subject': 'English'},
 {'grade': 'GD3',
  'index': '128-4',
  'level': 'G',
  'name': 'c0a78d6cba',
  'natScore': 69,
  'order': 1,
  'subject': 'Maths'},
 {'grade': 'GC2',
  'index': '128-14',
  'level': 'G',
  'name': 'c0a78d6cba',
  'natScore': 55,
  'order': 1,
  'subject': 'Ag. Science'},
 {'grade': 'GD1',
  'index': '128-15',
  'level': 'G',
  'name': 'c0a78d6cba',
  'natScore': 12,
  'order': 1,
  'subject': 'Biology'},
 {'grade': 'GC2',
  'index': '128-17',
  'level': 'G',
  'name': 'c0a78d6cba',
  'natScore': 71,
  'order': 1,
  'subject': 'Construction Studies'},
 {'grade': 'AB1',
  'index': '129-2',
  'level': 'A',
  'name': '711690fedd',
  'natScore': 25,
  'order': 1,
  'subject': 'Irish'},
 {'grade': 'AA2',
  'index': '129-3',
  'level': 'A',
  'name': '711690fedd',
  'natScore': 23,
  'order': 1,
  'subject': 'English'},
 {'grade': 'GB2',
  'index': '129-4',
  'level': 'G',
  'name': '711690fedd',
  'natScore': 72,
  'order': 1,
  'subject': 'Maths'},
 {'grade': 'AA2',
  'index': '129-6',
  'level': 'A',
  'name': '711690fedd',
  'natScore': 43,
  'order': 1,
  'subject': 'Geography'},
 {'grade': 'AB1',
  'index': '129-8',
  'level': 'A',
  'name': '711690fedd',
  'natScore': 91,
  'order': 1,
  'subject': 'German'},
 {'grade': 'AA2',
  'index': '129-15',
  'level': 'A',
  'name': '711690fedd',
  'natScore': 23,
  'order': 1,
  'subject': 'Biology'},
 {'grade': 'AB2',
  'index': '129-20',
  'level': 'A',
  'name': '711690fedd',
  'natScore': 51,
  'order': 1,
  'subject': 'Economics'},
 {'grade': 'AA2',
  'index': '130-2',
  'level': 'A',
  'name': 'eee2a82bc3',
  'natScore': 43,
  'order': 1,
  'subject': 'Irish'},
 {'grade': 'AC3',
  'index': '130-3',
  'level': 'A',
  'name': 'eee2a82bc3',
  'natScore': 18,
  'order': 1,
  'subject': 'English'},
 {'grade': 'GC2',
  'index': '130-4',
  'level': 'G',
  'name': 'eee2a82bc3',
  'natScore': 60,
  'order': 1,
  'subject': 'Maths'},
 {'grade': 'AB2',
  'index': '130-6',
  'level': 'A',
  'name': 'eee2a82bc3',
  'natScore': 5,
  'order': 1,
  'subject': 'Geography'},
 {'grade': 'AA2',
  'index': '130-8',
  'level': 'A',
  'name': 'eee2a82bc3',
  'natScore': 5,
  'order': 1,
  'subject': 'German'},
 {'grade': 'AB3',
  'index': '130-14',
  'level': 'A',
  'name': 'eee2a82bc3',
  'natScore': 86,
  'order': 1,
  'subject': 'Ag. Science'},
 {'grade': 'AC1',
  'index': '130-15',
  'level': 'A',
  'name': 'eee2a82bc3',
  'natScore': 33,
  'order': 1,
  'subject': 'Biology'},
 {'grade': 'GB2',
  'index': '131-2',
  'level': 'G',
  'name': '8d8e37aeff',
  'natScore': 6,
  'order': 1,
  'subject': 'Irish'},
 {'grade': 'AC1',
  'index': '131-3',
  'level': 'A',
  'name': '8d8e37aeff',
  'natScore': 42,
  'order': 1,
  'subject': 'English'},
 {'grade': 'GB1',
  'index': '131-4',
  'level': 'G',
  'name': '8d8e37aeff',
  'natScore': 79,
  'order': 1,
  'subject': 'Maths'},
 {'grade': 'AB3',
  'index': '131-6',
  'level': 'A',
  'name': '8d8e37aeff',
  'natScore': 0,
  'order': 1,
  'subject': 'Geography'},
 {'grade': 'AB2',
  'index': '131-14',
  'level': 'A',
  'name': '8d8e37aeff',
  'natScore': 54,
  'order': 1,
  'subject': 'Ag. Science'},
 {'grade': 'AB2',
  'index': '131-19',
  'level': 'A',
  'name': '8d8e37aeff',
  'natScore': 5,
  'order': 1,
  'subject': 'Business'},
 {'grade': 'AB3',
  'index': '131-20',
  'level': 'A',
  'name': '8d8e37aeff',
  'natScore': 70,
  'order': 1,
  'subject': 'Economics'},
 {'grade': 'GB2',
  'index': '132-2',
  'level': 'G',
  'name': 'b34716b2ed',
  'natScore': 18,
  'order': 1,
  'subject': 'Irish'},
 {'grade': 'GB2',
  'index': '132-3',
  'level': 'G',
  'name': 'b34716b2ed',
  'natScore': 35,
  'order': 1,
  'subject': 'English'},
 {'grade': 'GA2',
  'index': '132-4',
  'level': 'G',
  'name': 'b34716b2ed',
  'natScore': 91,
  'order': 1,
  'subject': 'Maths'},
 {'grade': 'AB2',
  'index': '132-6',
  'level': 'A',
  'name': 'b34716b2ed',
  'natScore': 19,
  'order': 1,
  'subject': 'Geography'},
 {'grade': 'AB2',
  'index': '132-8',
  'level': 'A',
  'name': 'b34716b2ed',
  'natScore': 35,
  'order': 1,
  'subject': 'German'},
 {'grade': 'AC1',
  'index': '132-15',
  'level': 'A',
  'name': 'b34716b2ed',
  'natScore': 55,
  'order': 1,
  'subject': 'Biology'},
 {'grade': 'AB1',
  'index': '132-16',
  'level': 'A',
  'name': 'b34716b2ed',
  'natScore': 11,
  'order': 1,
  'subject': 'Engineering'},
 {'grade': 'GB3',
  'index': '133-2',
  'level': 'G',
  'name': '6a414bcb3a',
  'natScore': 20,
  'order': 1,
  'subject': 'Irish'},
 {'grade': 'AC3',
  'index': '133-3',
  'level': 'A',
  'name': '6a414bcb3a',
  'natScore': 72,
  'order': 1,
  'subject': 'English'},
 {'grade': 'GC2',
  'index': '133-4',
  'level': 'G',
  'name': '6a414bcb3a',
  'natScore': 11,
  'order': 1,
  'subject': 'Maths'},
 {'grade': 'AB3',
  'index': '133-6',
  'level': 'A',
  'name': '6a414bcb3a',
  'natScore': 100,
  'order': 1,
  'subject': 'Geography'},
 {'grade': 'AC3',
  'index': '133-10',
  'level': 'A',
  'name': '6a414bcb3a',
  'natScore': 57,
  'order': 1,
  'subject': 'Art'},
 {'grade': 'AC2',
  'index': '133-14',
  'level': 'A',
  'name': '6a414bcb3a',
  'natScore': 57,
  'order': 1,
  'subject': 'Ag. Science'},
 {'grade': 'AD3',
  'index': '133-15',
  'level': 'A',
  'name': '6a414bcb3a',
  'natScore': 4,
  'order': 1,
  'subject': 'Biology'},
 {'grade': 'GC1',
  'index': '134-2',
  'level': 'G',
  'name': 'c1de5f864f',
  'natScore': 14,
  'order': 1,
  'subject': 'Irish'},
 {'grade': 'AD1',
  'index': '134-3',
  'level': 'A',
  'name': 'c1de5f864f',
  'natScore': 32,
  'order': 1,
  'subject': 'English'},
 {'grade': 'GC3',
  'index': '134-4',
  'level': 'G',
  'name': 'c1de5f864f',
  'natScore': 33,
  'order': 1,
  'subject': 'Maths'},
 {'grade': 'AC3',
  'index': '134-6',
  'level': 'A',
  'name': 'c1de5f864f',
  'natScore': 17,
  'order': 1,
  'subject': 'Geography'},
 {'grade': 'GC1',
  'index': '134-7',
  'level': 'G',
  'name': 'c1de5f864f',
  'natScore': 46,
  'order': 1,
  'subject': 'French'},
 {'grade': 'AC3',
  'index': '134-9',
  'level': 'A',
  'name': 'c1de5f864f',
  'natScore': 51,
  'order': 1,
  'subject': 'Spanish'},
 {'grade': 'AC3',
  'index': '134-14',
  'level': 'A',
  'name': 'c1de5f864f',
  'natScore': 79,
  'order': 1,
  'subject': 'Ag. Science'},
 {'grade': 'AC1',
  'index': '134-22',
  'level': 'A',
  'name': 'c1de5f864f',
  'natScore': 64,
  'order': 1,
  'subject': 'Home Ec.'},
 {'grade': 'GC1',
  'index': '135-3',
  'level': 'G',
  'name': 'a2f722ffb5',
  'natScore': 65,
  'order': 1,
  'subject': 'English'},
 {'grade': 'AE',
  'index': '135-6',
  'level': 'A',
  'name': 'a2f722ffb5',
  'natScore': 13,
  'order': 1,
  'subject': 'Geography'},
 {'grade': 'GE',
  'index': '135-14',
  'level': 'G',
  'name': 'a2f722ffb5',
  'natScore': 62,
  'order': 1,
  'subject': 'Ag. Science'},
 {'grade': 'AD2',
  'index': '135-16',
  'level': 'A',
  'name': 'a2f722ffb5',
  'natScore': 77,
  'order': 1,
  'subject': 'Engineering'},
 {'grade': 'GC3',
  'index': '135-20',
  'level': 'G',
  'name': 'a2f722ffb5',
  'natScore': 6,
  'order': 1,
  'subject': 'Economics'},
 {'grade': 'GC2',
  'index': '136-3',
  'level': 'G',
  'name': 'a390a26c50',
  'natScore': 26,
  'order': 1,
  'subject': 'English'},
 {'grade': 'GC3',
  'index': '136-6',
  'level': 'G',
  'name': 'a390a26c50',
  'natScore': 91,
  'order': 1,
  'subject': 'Geography'},
 {'grade': 'GC3',
  'index': '136-10',
  'level': 'G',
  'name': 'a390a26c50',
  'natScore': 76,
  'order': 1,
  'subject': 'Art'},
 {'grade': 'GC1',
  'index': '136-15',
  'level': 'G',
  'name': 'a390a26c50',
  'natScore': 15,
  'order': 1,
  'subject': 'Biology'},
 {'grade': 'GC1',
  'index': '136-21',
  'level': 'G',
  'name': 'a390a26c50',
  'natScore': 92,
  'order': 1,
  'subject': 'Music'},
 {'grade': 'AC1',
  'index': '137-2',
  'level': 'A',
  'name': '1b43d051ec',
  'natScore': 8,
  'order': 1,
  'subject': 'Irish'},
 {'grade': 'AD2',
  'index': '137-3',
  'level': 'A',
  'name': '1b43d051ec',
  'natScore': 7,
  'order': 1,
  'subject': 'English'},
 {'grade': 'GC1',
  'index': '137-4',
  'level': 'G',
  'name': '1b43d051ec',
  'natScore': 20,
  'order': 1,
  'subject': 'Maths'},
 {'grade': 'AC2',
  'index': '137-9',
  'level': 'A',
  'name': '1b43d051ec',
  'natScore': 26,
  'order': 1,
  'subject': 'Spanish'},
 {'grade': 'AC3',
  'index': '137-14',
  'level': 'A',
  'name': '1b43d051ec',
  'natScore': 38,
  'order': 1,
  'subject': 'Ag. Science'},
 {'grade': 'AC3',
  'index': '137-15',
  'level': 'A',
  'name': '1b43d051ec',
  'natScore': 23,
  'order': 1,
  'subject': 'Biology'},
 {'grade': 'AC2',
  'index': '137-21',
  'level': 'A',
  'name': '1b43d051ec',
  'natScore': 44,
  'order': 1,
  'subject': 'Music'},
 {'grade': 'GE',
  'index': '138-2',
  'level': 'G',
  'name': '73e51aebb7',
  'natScore': 12,
  'order': 1,
  'subject': 'Irish'},
 {'grade': 'GB3',
  'index': '138-3',
  'level': 'G',
  'name': '73e51aebb7',
  'natScore': 19,
  'order': 1,
  'subject': 'English'},
 {'grade': 'GE',
  'index': '138-4',
  'level': 'G',
  'name': '73e51aebb7',
  'natScore': 88,
  'order': 1,
  'subject': 'Maths'},
 {'grade': 'AE',
  'index': '138-6',
  'level': 'A',
  'name': '73e51aebb7',
  'natScore': 89,
  'order': 1,
  'subject': 'Geography'},
 {'grade': 'AE',
  'index': '138-19',
  'level': 'A',
  'name': '73e51aebb7',
  'natScore': 22,
  'order': 1,
  'subject': 'Business'},
 {'grade': 'AD2',
  'index': '138-22',
  'level': 'A',
  'name': '73e51aebb7',
  'natScore': 80,
  'order': 1,
  'subject': 'Home Ec.'},
 {'grade': 'AB3',
  'index': '139-2',
  'level': 'A',
  'name': '1aab1c0bb2',
  'natScore': 82,
  'order': 1,
  'subject': 'Irish'},
 {'grade': 'GB2',
  'index': '139-3',
  'level': 'G',
  'name': '1aab1c0bb2',
  'natScore': 5,
  'order': 1,
  'subject': 'English'},
 {'grade': 'GC3',
  'index': '139-4',
  'level': 'G',
  'name': '1aab1c0bb2',
  'natScore': 79,
  'order': 1,
  'subject': 'Maths'},
 {'grade': 'GC1',
  'index': '139-6',
  'level': 'G',
  'name': '1aab1c0bb2',
  'natScore': 58,
  'order': 1,
  'subject': 'Geography'},
 {'grade': 'GD1',
  'index': '139-8',
  'level': 'G',
  'name': '1aab1c0bb2',
  'natScore': 19,
  'order': 1,
  'subject': 'German'},
 {'grade': 'GC3',
  'index': '139-14',
  'level': 'G',
  'name': '1aab1c0bb2',
  'natScore': 14,
  'order': 1,
  'subject': 'Ag. Science'},
 {'grade': 'GB3',
  'index': '139-15',
  'level': 'G',
  'name': '1aab1c0bb2',
  'natScore': 27,
  'order': 1,
  'subject': 'Biology'},
 {'grade': 'GD3',
  'index': '140-3',
  'level': 'G',
  'name': 'b9b76cdca8',
  'natScore': 92,
  'order': 1,
  'subject': 'English'},
 {'grade': 'GF',
  'index': '140-4',
  'level': 'G',
  'name': 'b9b76cdca8',
  'natScore': 50,
  'order': 1,
  'subject': 'Maths'},
 {'grade': 'GE',
  'index': '140-6',
  'level': 'G',
  'name': 'b9b76cdca8',
  'natScore': 31,
  'order': 1,
  'subject': 'Geography'},
 {'grade': 'AE',
  'index': '140-10',
  'level': 'A',
  'name': 'b9b76cdca8',
  'natScore': 41,
  'order': 1,
  'subject': 'Art'},
 {'grade': 'GF',
  'index': '140-22',
  'level': 'G',
  'name': 'b9b76cdca8',
  'natScore': 31,
  'order': 1,
  'subject': 'Home Ec.'}])




var ranking = {
     "AA1":1,
    "AA2":2,
    "AB1":3,
    "AB2":4,
    "AB3":5,
    "AC1":6,
    "AC2":7,
    "AC3":8,
    "AD1":9,
    "AD2":10,
    "AD3":11,
    "AE" :12,
    "AF" :13,
    "ANG" :14,
    "GA1":15,
    "GA2":16,
    "GB1":17,
    "GB2":18,
    "GB3":19,
    "GC1":20,
    "GC2":21,
    "GC3":22,
    "GD1":23,
    "GD2":24,
    "GD3":25,
    "GE" :26,
    "GF" :27,
    "BA1":28,
    "BA2":29,
    "BB1":30,
    "BB2":31,
    "BB3":32,
    "BC1":33,
    "BC2":34,
    "BC3":35,
    "BD1":36,
    "BD2":37,
    "BD3":38,
    "BE" :39
}


var national = {
    "AA1":0.0125,
    "AA2":0.025,
    "AB1":0.025,
    "AB2":0.05,
    "AB3":0.075,
    "AC1":0.075,
    "AC2":0.05,
    "AC3":0.05,
    "AD1":0.025,
    "AD2":0.025,
    "AD3":0.0125,
    "AE" :0.0125,
    "GA1":0.0125,
    "GA2":0.025,
    "GB1":0.025,
    "GB2":0.05,
    "GB3":0.075,
    "GC1":0.075,
    "GC2":0.05,
    "GC3":0.05,
    "GD1":0.025,
    "GD2":0.025,
    "GD3":0.0125,
    "GE" :0.0125,
    "BA1":0.015,
    "BA2":0.001,
    "BB1":0.001,
    "BB2":0.001,
    "BB3":0.001,
    "BC1":0.001,
    "BC2":0.001,
    "BC3":0.001,
    "BD1":0.001,
    "BD2":0.001,
    "BD3":0.001,
}


// Variables
var fudge = 10;

var dimensionGrade = gradeDict.dimension(student_grade => student_grade.grade)
var countByGrade = dimensionGrade.group().reduceCount()
// console.log(countByGrade.all())

// Attempt to get %
var avgGradeNational = dimensionGrade.group()
var reducerloc = reductio()
    .count(true)
    .sum(function(d) { return national[d.grade]; })
    .avg(true);
reducerloc(avgGradeNational)

var dimensionName = gradeDict.dimension(student_grade => student_grade.name)
var countByName = dimensionName.group().reduceCount()
// console.log("Name count")
// console.log(countByName.all())
var dimensionIndex = gradeDict.dimension(student_grade => student_grade.index)
var countByIndex = dimensionIndex.group().reduceCount()


// var dimensionRank = supermarketItems.dimension(student_grade => student_grade.rank)

var natAvg = dimensionGrade.group()
var reducer = reductio()
    .count(true)
    .sum(function(d) { return national[d.grade]; })
    .avg(true);
reducer(natAvg)


let dimensionSubject = gradeDict.dimension(student_grade => student_grade.subject)
let countBySubject = dimensionSubject.group().reduceCount()

// Group for bix plot
// var cat = dimensionGrade.group()
// var catReducer = reductio()
//     .count(true)
//     .sum(function(d) { return d.natScore; })
//     .avg(true);
// catReducer(cat)

// Box plot witchcraft
catGroup      = dimensionGrade.group().reduce(
  function(p,v) {
    p.push(v.natScore);
    return p;
  },
  function(p,v) {
    p.splice(p.indexOf(v.natScore), 1);
    return p;
  },
  function() {
    return [];
  }
);



// Composite chart


var composite = dc.compositeChart("#composite_grades2");
 composite
        .width(window.innerWidth*3/5 - fudge)
        .height(window.innerHeight*2/3 - fudge)
        .x(d3.scaleBand())
        .xUnits(dc.units.ordinal)
        .yAxisLabel("The Y Axis")
        .group(avgGradeNational)
        ._rangeBandPadding(1)
        .legend(dc.legend().x(80).y(20).itemHeight(13).gap(5))
            .ordering(function(d) { return -ranking[d.key]; })
        .renderHorizontalGridLines(true)
        .compose([
            // dc.barChart(composite)
            //     .dimension(dimensionGrade)
            //     .colors('red')
            //     .centerBar(true)
            //     .gap(1)
            //     .group(avgGradeNational, "School Values")
            //     .valueAccessor(function (p) {return p.value.count/countByIndex.all().length}),
            dc.barChart(composite)
                .dimension(dimensionGrade)
                .colors('#87a8dd')
                .centerBar(true)
                .gap(1)
                .group(avgGradeNational, "National Values")
                .valueAccessor(function (p) {return p.value.avg}),
            // dc.lineChart(composite)
            //     .dimension(dimensionGrade)
            //     .colors('red')
            //     .group(avgGradeNational, "National Line")
            //     .valueAccessor(function (p) {return p.value.avg}),
            dc.lineChart(composite)
                .dimension(dimensionGrade)
                .colors('red')
                .group(avgGradeNational, "School values")
                .valueAccessor(function (p) {return p.value.count/countByIndex.all().length})
                .dashStyle([2,2])
            ]);

composite.render();


console.log(catGroup.all())



// I think it should be total count, over count of entries remaining
// While the other is sum of national values/ sum or



// 
// console.log(catGroup.all())
// 
// 
// 
var boxChart = dc.boxPlot("#box-test")
  
  boxChart
    .width(768)
    .height(480)
    .margins({top: 10, right: 50, bottom: 30, left: 50})
    .dimension(dimensionGrade)
    .group(catGroup)
    .x(d3.scaleBand())
    .xUnits(dc.units.ordinal)
    // .elasticY(true);

  // this demonstrates solving elasticX manually, avoiding the
  // bug where the rightmost bar/box is not displayed, #792
  // function calc_domain(chart) {
  //     var min = d3.min(chart.group().all(), function(kv) { return kv.key; }),
  //         max = d3.max(chart.group().all(), function(kv) { return kv.key; });
  //     max = d3.timeMonth.offset(max, 1);
  //     chart.x().domain([min, max]);
  // }
  // boxChart.on('preRender', calc_domain);
  // boxChart.on('preRedraw', calc_domain);

  boxChart.render()